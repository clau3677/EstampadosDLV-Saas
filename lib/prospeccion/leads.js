...
export async function listLeads({ page = 1, pageSize = 50, category, commune, state, source, minScore, maxScore, q, sortBy = 'score', phoneType } = {}) {
  ...
  if (phoneType) {
    if (phoneType === 'celular') filter.phone = { $regex: /^\+?569\d{8}$|^9\d{8}$|^9\s?\d{4}\s?\d{4}$|^569\d{8}$/ };
    else if (phoneType === 'fijo') filter.phone = { $exists: true, $ne: null, $not: /^\+?569\d{8}$|^9\d{8}$|^9\s?\d{4}\s?\d{4}$|^569\d{8}$/ };
  ...
  const SORTS = {
    website: { website: -1, 'score.final': -1, createdAt: -1 },
