 ...
 */
export async function listLeads({ page = 1, pageSize = 50, category, commune, state, source, minScore, maxScore, q } = {}) {
  ...
  const [items, total] = await Promise.all([
    leads.find(filter).sort({ 'score.final': -1, createdAt: -1 }).skip(skip).limit(pageSize).toArray(),
    leads.countDocuments(filter),
  ]);
  return { items, page, pageSize, total, filters: { category, commune, state, source, minScore, maxScore, q } };
}
