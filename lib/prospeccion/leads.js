 ...
 */
export async function listLeads({ page = 1, pageSize = 50, category, commune, state, source, minScore, maxScore, q, sortBy = 'score', phoneType } = {}) {
  ...
  // Filtro por tipo de contacto (build114): celular / fijo / correo / ninguno
  if (phoneType) {
    if (phoneType === 'celular') filter.phone = { $regex: /^\+?569\d{8}$|^56\s?9\s?\d{4}\s?\d{4}$|^9\d{8}$|^\+569/ };
    else if (phoneType === 'fijo') filter.phone = { $exists: true, $ne: null, $not: /^\+?569\d{8}$|^56\s?9\s?\d{4}\s?\d{4}$|^9\d{8}$|^\+569/ };
    else if (phoneType === 'correo') filter.email = { $exists: true, $ne: null, $regex: /@/ };
    else if (phoneType === 'sin_correo') filter.$or = [{ email: { $exists: false } }, { email: null }, { email: '' }];
  }
  ...
  return { items, page, pageSize, total, filters: { category, commune, state, source, minScore, maxScore, q, sortBy, phoneType } };
}

/**
 * Aprueba en masa prospectos (build114): cambia a aprobado_contacto todos los
 * que coincidan con el filtro actual, usando solo transiciones permitidas.
 * Devuelve cuántos se aprobaron y cuántos quedaron sin cambio.
 */
export async function bulkApproveLeads({ category, commune, state, source, minScore, maxScore, q, phoneType, limit = 5000 } = {}) {
  const leads = await coll(COLLECTIONS.PRO_LEADS);
  const filter = {};
  if (category) filter.category = category;
  if (commune) filter.commune = commune;
  if (source) filter.source = source;
  if (phoneType === 'celular') filter.phone = { $regex: /^\+?569/ };
  else if (phoneType === 'fijo') filter.phone = { $exists: true, $ne: null, $not: /^\+?569/ };
  else if (phoneType === 'correo') filter.email = { $exists: true, $ne: null, $regex: /@/ };
  if (minScore !== null && minScore !== undefined) filter['score.final'] = { ...(filter['score.final'] || {}), $gte: minScore };
  if (maxScore !== null && maxScore !== undefined) filter['score.final'] = { ...(filter['score.final'] || {}), $lte: maxScore };
  if (q) filter.name = { $regex: q, $options: 'i' };

  const cursor = leads.find(filter).limit(limit);
  let approved = 0;
  let skipped = 0;
  for await (const lead of cursor) {
    const allowed = TRANSITIONS[lead.state] || [];
    if (allowed.includes('aprobado_contacto')) {
      await leads.updateOne({ id: lead.id }, { $set: { state: 'aprobado_contacto', updatedAt: new Date() } });
      approved += 1;
    } else {
      skipped += 1;
    }
  ...
  await logAudit({
    ...
    entityType: 'leads',
    entityId: 'bulk',
    details: { bulkApprove: true, filters: { category, commune, state, source, minScore, maxScore, q, phoneType }, approved, skipped },
  });
  return { approved, skipped, limit };
}
