...
export const DEDUP_KINDS = ['email', 'domain', 'phone', 'nameCommune'];
 ...
 */
export async function findDuplicate({ name, commune, email, phone, website }) {
  const keys = buildDedupKeys({ name, commune, email, phone, website });
...
export async function upsertLead(doc) {
  ...
  const leads = await coll(COLLECTIONS.PRO_LEADS);
  // Prioridad: email exacto → dominio → teléfono → nombre+comuna
  const conditions = [
    ...(keys.emailLower ? [{ emailLower: keys.emailLower }] : []),
    ...(keys.phoneNormalized ? [{ phoneNormalized: keys.phoneNormalized }] : []),
    ...(keys.emailDomain ? [{ emailDomain: keys.emailDomain }] : []),
