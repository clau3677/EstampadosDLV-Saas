...
function hostMatches(siteHost, emailHost) {
  ...
  return e.endsWith('.' + s) || s.endsWith('.' + e);
}
...
const CONTACT_PATHS = ['', '/contacto', '/contactenos', '/contact-us', '/contacto-nosotros', '/informacion', '/quienes-somos', '/sobre-nosotros'];
...
/**
 * Descarga una URL con timeout y User-Agent de navegador normal.
 */
...
export async function enrichLeadsWithEmails({ category, commune, minScore, q, phoneType, limit = 100 } = {}) {
  ...
  for (const lead of leads.items) {
    ...
    try {
      const { email, source } = await findBusinessEmail(lead.website);
      if (email) {
  ...
  await (await import('./audit.js')).logAudit({
    ...
    details: {
      ...
      limit,
      leadsWithWebsite: leads.items.filter(l => l.website).length,
      emailsFound: found,
      noEmail: missing,
      processed,
    },
  });
  return { total: leads.items.length, found, missing, processed };
}
