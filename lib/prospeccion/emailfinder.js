...
export async function enrichLeadsWithEmails({ category, commune, minScore, q, phoneType, limit = 100 } = {}) {
  ...
  const now = Date.now();
  const work = leads.items
    .filter(l => l.website && !/instagram\.com|facebook\.com|wa\.me|tiktok\.com/i.test(l.website) && !isSiteBuilder(l.website))
    .filter(l => {
      const tried = l.emailTriedAt;
      if (!tried) return true;
      const t = tried.getTime ? tried.getTime() : new Date(tried).getTime();
      return now - t > RETRY_MS;
    });
