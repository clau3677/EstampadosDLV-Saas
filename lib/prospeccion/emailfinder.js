...
async function runFindBusinessEmail(candidates) {
  ...
  for (const url of candidates) {
    // Fase 1: Intento rápido con curl
    let html = await fetchPage(url);
    
    // Fase 2: Si curl falla o devuelve poco contenido, intentar con Obscura (headless)
    if (!html || html.length < 2000) {
      try {
        const { fetchWithObscura } = await import('./obscura-helper.js');
        const obscuraHtml = await fetchWithObscura(url);
        if (obscuraHtml && obscuraHtml.length > (html?.length || 0)) {
          html = obscuraHtml;
        }
      } catch (e) {
        // Silencioso, fallback a lo que tengamos
      }
    }
...
export async function enrichLeadsWithEmails({ category, commune, minScore, q, phoneType, limit = 100 } = {}) {
  ...
  const query = {
    ...
    website: { $exists: true, $nin: [null, ''], $not: /instagram\.com|facebook\.com|wa\.me|tiktok\.com|wixsite\.com|wix\.com|ueniweb\.com|webnode\.com|jimdo|notion\.site|sites\.google\.com|site123\.me|durable\.co|carrd\.co|strikingly\.com|square\.site|godaddysites\.com|homestead\.com|weebly\.com|blogspot\.com|webflow\.io|business\.site|linktr\.ee/i },
    $and: [
      { $or: [
  ...
  const leadsItems = await leadsColl
    .find(query)
