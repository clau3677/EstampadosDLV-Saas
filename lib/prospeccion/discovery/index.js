...
import { runDiscovery as runGooglePlaces, isGooglePlacesEnabled } from './googlePlaces.js';
import { runDiscovery as runOpenStreetMap, isGooglePlacesEnabled as isOsmEnabled } from './openStreetMap.js';
...
const PROVIDERS = {
  ...
  scraper: runScraper,
  manual: runManual,
  google_places: runGooglePlaces,
  openstreetmap: runOpenStreetMap, // proveedor gratuito (Overpass API) — build120
};

/**
 * Auto-selecciona el proveedor por defecto:
 *  - google_places si la API de Google está habilitada (datos reales)
 *  - simulated como fallback seguro
 */
// build120: OpenStreetMap (Overpass API) es el proveedor REAL por defecto.
// Es 100% gratuito; Google Places API (New) quedó descartada por su alto costo.
function defaultProvider() {
  if (isOsmEnabled()) return 'openstreetmap';
...
export async function runDiscoveryForCampaign({ campaignId, providerOverride = null, manualBusinesses = [], actorId = null } = {}) {
  ...
  const providerName = providerOverride || campaign.provider || defaultProvider();
