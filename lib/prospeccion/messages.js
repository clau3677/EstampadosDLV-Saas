...
export function isSimulationMode() {
  // Simulación ON por defecto. Se apaga si hay un proveedor real verificado:
  // 1) SMTP de producción (estampadosdlv@gmail.com ya probado en el módulo Emails SMTP)
  // 2) Resend API key
  ...
  if (smtpReady || resendReady) return false;
  return true;
}
...
export async function approveMessageBatch({ campaignId, campaignLeadIds = [], actorId = null, actorName = null } = {}) {
  ...
  for (const row of rows) {
    try {
      ...
      if ((lead.score?.final ?? 0) < (campaign.outreach?.minScore ?? 60)) {
        skipped.push({ campaignLeadId: row.id, reason: 'score bajo el mínimo de la campaña' }); continue;
      }

      const channel = (campaign.outreach?.channel || 'email').startsWith('whatsapp') ? 'whatsapp' : 'email';
      const rendered = renderTemplate(channel, lead.category || 'otros', lead);
      // WhatsApp usa el teléfono del prospecto; sin teléfono no se puede enviar.
      if (channel === 'whatsapp' && !lead.phone) {
        skipped.push({ campaignLeadId: row.id, reason: 'sin teléfono para WhatsApp' }); continue;
      }
      ...
      const msg = {
        ...
        recipient: channel === 'email' ? (lead.email || null) : (lead.phone || null),
        emailLower: channel === 'email' && lead.email ? String(lead.email).toLowerCase() : null,
        status: channel === 'whatsapp' ? 'pendiente' : (isSimulationMode() ? 'simulado' : 'pendiente'),
        simulationMode: channel === 'email' ? isSimulationMode() : false,
        createdAt: new Date(),
      };
      await messages.insertOne(msg);
      created.push(msg.id);

      // Crear job de envío (respetando ventana horaria y límite diario)
      const job = {
        id: uuidv4(),
        type: 'message.send',
        status: 'pending',
  ...
  return { created: created.length, skipped, channel: campaign.outreach?.channel || 'email' };
}
