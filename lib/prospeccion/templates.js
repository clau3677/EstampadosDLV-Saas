...
export const SERVICE_BY_CATEGORY = {
  restaurantes: 'delantales, ropa de trabajo y poleras ya personalizadas para tu equipo, listas para usar',
  cafeterias: 'delantales y polerones ya personalizados con el logo de tu cafetería',
  bares: 'polerones y gorras ya personalizados con tu logo para el equipo y clientes',
  ...
  otros: 'poleras, polerones, gorras y merchandising ya personalizados con tu diseño',
};
...
const TEMPLATE_DEFINITIONS = [
  { code: 'general', name: 'Presentación general', description: 'Prendas, uniformes, ropa de trabajo y productos personalizados para negocios locales.' },
  { code: 'marcas', name: 'Marcas de ropa', description: 'Producción de prendas, colecciones y pliegos DTF para marcas y talleres.' },
  { code: 'deporte', name: 'Gimnasios y deporte', description: 'Uniformes y prendas personalizadas para equipos, academias y comunidades deportivas.' },
  { code: 'empresas', name: 'Empresas y uniformes', description: 'Uniformes corporativos, ropa de trabajo y prendas para equipos de empresa.' },
  { code: 'merchandising', name: 'Merchandising y regalos', description: 'Productos promocionales, regalos corporativos y personalización DTF UV.' },
];

const TEMPLATE_CODES = new Set(TEMPLATE_DEFINITIONS.map((item) => item.code));

/** Plantillas seleccionables por campaña. Los contenidos siguen centralizados en este archivo. */
export function listMessageTemplates(channel = null) {
  const channels = channel ? [channel] : ['email', 'whatsapp'];
  return channels
    .filter((item) => item === 'email' || item === 'whatsapp')
    .flatMap((item) => TEMPLATE_DEFINITIONS.map((template) => ({
      id: `${item}-${template.code}`,
      channel: item,
      code: template.code,
      name: template.name,
...
export function isValidTemplateId(templateId, channel) {
  if (!templateId || !['email', 'whatsapp'].includes(channel)) return false;
  const prefix = `${channel}-`;
  return String(templateId).startsWith(prefix) && TEMPLATE_CODES.has(String(templateId).slice(prefix.length));
}

function resolveTemplate(channel, category, templateId) {
  const code = isValidTemplateId(templateId, channel)
    ? String(templateId).slice(`${channel}-`.length)
    : templateForCategory(category);
  return { code, id: `${channel}-${code}` };
}
...
const EMAIL_BY_TPL = {
  general: (v) => emailHtml({
    ...
    highlight: `<strong>Prendas terminadas, listas para usar</strong>
      <p>Nosotros imprimimos y entregamos tus prendas y uniformes listos, con buena resistencia al lavado y entrega rápida.</p>`,
    items: ['Poleras, polerones y buzos personalizados con tu logo.',
      'Ropa de trabajo y delantales personalizados para tu equipo.',
      'Impresión DTF UV para llaveros, vasos y botellas con tu marca.',
      'Producción desde una unidad, con despacho a todo Chile.'],
...
const WA_BY_TPL = {
  general: (v) => [
    ...
    '',
    `Ayudamos a negocios como *${v.business_name}* entregando prendas ya personalizadas y listas para usar: poleras, polerones, gorras, uniformes, ropa de trabajo y delantales con su logo, más vasos, botellas, llaveros y merchandising con su marca.`,
    '',
...
export const EMAIL_TEMPLATE = {
  subject: (category, v, templateId = null) => {
    const { code } = resolveTemplate('email', category, templateId);
    const gen = SUBJECT_BY_TPL[code] || SUBJECT_BY_TPL.general;
    return (gen.subject || SUBJECT_BY_TPL.general.subject)(v);
  },
  body: (category, v, templateId = null) => {
    const { code } = resolveTemplate('email', category, templateId);
    return (EMAIL_BY_TPL[code] || EMAIL_BY_TPL.general)(v);
  },
  /** Versión texto plano del correo (para clientes de correo sin HTML). */
  plainText: (category, v, templateId = null) => {
    const { code } = resolveTemplate('email', category, templateId);
    return (WA_BY_TPL[code] || WA_BY_TPL.general)(v)
      .replace(/\*([^*]+)\*/g, '$1')
...
export const WHATSAPP_TEMPLATE = {
  body: (category, v, templateId = null) => {
    const { code } = resolveTemplate('whatsapp', category, templateId);
    return (WA_BY_TPL[code] || WA_BY_TPL.general)(v)
...
export function renderTemplate(channel, category, lead, opts = {}) {
  const v = {
    ...
    contact_name: (() => {
      const c = lead.contactName || lead.contact_name || '';
      const first = c.split(' ')[0];
      // Si el "contacto" es el nombre del negocio, usar el nombre completo del negocio (más profesional)
      if (!c || first === lead.name?.split(' ')[0]) return lead.name || '';
      return first || lead.name || '';
    })(),
  ...
  const selected = resolveTemplate(channel === 'whatsapp' ? 'whatsapp' : 'email', category, opts.templateId);
  if (channel === 'whatsapp') {
    return { templateId: selected.id, body: WHATSAPP_TEMPLATE.body(category, v, selected.id) };
  }
  return {
    templateId: selected.id,
    subject: EMAIL_TEMPLATE.subject(category, v, selected.id),
    body: EMAIL_TEMPLATE.body(category, v, selected.id),
    plainText: EMAIL_TEMPLATE.plainText(category, v, selected.id),
  };
}
