...
import { isValidTemplateId } from './templates.js';
...
export function normalizeCampaignInput(body) {
  ...
  const emailTemplateId = body.emailTemplateId || body.emailTemplate || null;
  const whatsappTemplateId = body.whatsappTemplateId || body.whatsappTemplate || null;
  if (emailTemplateId && !isValidTemplateId(emailTemplateId, 'email')) throw new Error('plantilla de email no válida');
  if (whatsappTemplateId && !isValidTemplateId(whatsappTemplateId, 'whatsapp')) throw new Error('plantilla de WhatsApp no válida');
  ...
  return {
    ...
    // Plantillas elegidas por canal. Si quedan en null, el motor usa el fallback por rubro.
    emailTemplateId,
    whatsappTemplateId,
    // Programación
    schedule: {
...
/**
 * Elimina una campaña equivocada únicamente si está en borrador o pausada y no tiene mensajes enviados.
 * Se conservan los prospectos globales; solo se borran las asignaciones, mensajes no enviados y jobs de la campaña.
 */
export async function deleteCampaign(id, actorId = null, actorName = null) {
  const campaigns = await coll(COLLECTIONS.PRO_CAMPAIGNS);
  ...
  const existing = await campaigns.findOne({ id });
  if (!existing) throw new Error('campaña no encontrada');
  if (!['borrador', 'pausada'].includes(existing.status)) {
    throw new Error('solo se pueden borrar campañas en borrador o pausadas');
  }

  const sentMessages = await messages.countDocuments({
    campaignId: id,
    $or: [
      { sentAt: { $exists: true, $ne: null } },
      { status: { $in: ['enviado', 'rebote'] } },
    ],
  });
  if (sentMessages > 0) {
    throw new Error('la campaña tiene mensajes enviados y debe conservarse como historial');
  }

  const [leadResult, messageResult, jobResult] = await Promise.all([
    campaignLeads.deleteMany({ campaignId: id }),
    messages.deleteMany({ campaignId: id }),
    jobs.deleteMany({ campaignId: id }),
  ]);
  await campaigns.deleteOne({ id });

  const deleted = {
    campaignLeads: leadResult.deletedCount || 0,
  ...
  await logAudit({
    ...
    entityId: id,
    details: { name: existing.name, status: existing.status, deleted },
  });
  return { id, deleted: true, counts: deleted };
}
