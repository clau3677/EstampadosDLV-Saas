...
const AUTONEST_GAP_MM   = 3;     // gap entre diseños en auto-nesting
...
// ----------------------------------------------------------------------------
function smartAutoNest(designs, canvasWidthMm, gap = AUTONEST_GAP_MM, allowRotate = true) {
...
export const useGangSheet = create((set, get) => ({
  ...
  reset: () => {
    ...
    set({
      ...
      snapEnabled: false, snapGridMm: 5, zoom: 1,
    });
  },
  ...
  // ------------------------------------------------------------------
  autoArrange: (opts = {}) => {
    const { allowRotate = true, gap = AUTONEST_GAP_MM } = opts;
    set((st) => ({
      designs: smartAutoNest(st.designs, st.canvasWidthMm, gap, allowRotate),
    }));
    get()._pushHistory();
  },
