...
function loadImage(src) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    // Sin crossOrigin: las imágenes son del mismo origen y no taintean el
    // canvas; con crossOrigin y headers de proxy inconsistentes fallaba la
    // carga y el análisis de transparencia.
    ...
    img.onerror = () => {
      ...
      retry.src = src + (src.includes('?') ? '&' : '?') + 'v=' + Date.now();
    };
    img.src = src;
  });
}
