...
import {
  ...
  tplOrderPacked, tplOrderHandedToCourier, tplOrderDelivered,
  tplPaymentApproved, tplPaymentRejected, tplReviewRequest,
  tplNewGangSheet, tplContestWinner, tplContestParticipation,
} from './templates';
...
export async function notifyOrderPackedByEmail({ order }) {
  if (!order) return { ok: false, reason: 'no_order' };
  const to = order.customerSnapshot?.email;
  const { subject, html, text } = tplOrderPacked({
    orderNumber: order.orderNumber,
    customerName: order.customerSnapshot?.name,
    deliveryMethod: order.deliveryMethod || 'pickup',
  });
...
export async function notifyOrderHandedToCourierByEmail({ order, carrier, trackingCode, trackingUrl }) {
  if (!order) return { ok: false, reason: 'no_order' };
  const to = order.customerSnapshot?.email;
  const { subject, html, text } = tplOrderHandedToCourier({
    orderNumber: order.orderNumber,
    customerName: order.customerSnapshot?.name,
    carrier,
    trackingCode,
  ...
  return safeSend({
    to, subject, html, text,
    event: 'order_handed_to_courier',
    orderId: order.id,
    orderNumber: order.orderNumber,
  });
}

export async function notifyOrderDeliveredByEmail({ order, proofUrl }) {
  if (!order) return { ok: false, reason: 'no_order' };
  const to = order.customerSnapshot?.email;
  const { subject, html, text } = tplOrderDelivered({
    orderNumber: order.orderNumber,
    customerName: order.customerSnapshot?.name,
    proofUrl,
  });
...
export async function notifyContestParticipationByEmail({ email, contestTitle, endDateLabel }) {
  if (!email || !isValidEmail(email)) return { ok: false, reason: 'invalid_email' };
  const { subject, html, text } = tplContestParticipation({ email, contestTitle, endDateLabel });
  return safeSend({
    to: email,
    subject,
    html,
    text,
    event: 'contest_participation_confirmation',
    note: 'Confirmación de participación en concurso',
  });
}
