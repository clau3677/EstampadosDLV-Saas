...
export function tplOrderPacked({ orderNumber, customerName, deliveryMethod }) {
  const name = customerName?.split(' ')[0] || 'Hola';
  const cta = deliveryMethod === 'shipping'
    ? 'Ya está empaquetado y esperando al transportista.'
    : 'Ya está empaquetado y listo para que lo pases a buscar.';
  const contentHtml = `
    <h1 style="margin:0 0 8px 0;font-size:22px;font-weight:700;">Pedido empaquetado 📦</h1>
    <p style="margin:0 0 20px 0;color:${BRAND.mutedColor};font-size:14px;">${escapeHtml(name)}, tu pedido ya está listo y protegido para su entrega.</p>
  ...
  const text = `${name}, tu pedido ${orderNumber} ya está empaquetado. — Estampados DLV`;

  return {
    subject: `Pedido ${orderNumber} empaquetado 📦`,
    html: shell({ title: `${orderNumber} empaquetado`, previewText: 'Tu pedido ya está protegido para su entrega', contentHtml }),
    text,
  };
}

export function tplOrderHandedToCourier({ orderNumber, customerName, carrier, trackingCode, trackingUrl }) {
  const name = customerName?.split(' ')[0] || 'Hola';
  const courierInfo = carrier ? `<p style="margin:12px 0;font-size:14px;"><strong>Transportista:</strong> ${escapeHtml(carrier)}</p>` : '';
  const trackInfo = trackingCode ? `<p style="margin:12px 0;font-size:14px;"><strong>Código de seguimiento:</strong> ${escapeHtml(trackingCode)}</p>` : '';
  const trackButton = trackingUrl ? `
    <div style="text-align:center;margin:20px 0;">
      <a href="${trackingUrl}" style="display:inline-block;padding:12px 24px;background:${BRAND.color};color:#ffffff;border-radius:10px;text-decoration:none;font-weight:700;font-size:14px;">Seguir mi pedido 🚚</a>
    </div>` : '';

  const contentHtml = `
    <h1 style="margin:0 0 8px 0;font-size:22px;font-weight:700;">¡Tu pedido va en camino! 🚚</h1>
    <p style="margin:0 0 20px 0;color:${BRAND.mutedColor};font-size:14px;">${escapeHtml(name)}, entregamos tu pedido al transportista.</p>
    ...
    <p style="margin:0;font-size:13px;color:${BRAND.mutedColor};">Los tiempos de entrega dependen del transportista, pero suele tardar entre 24 y 72 horas hábiles.</p>`;

  const text = `${name}, tu pedido ${orderNumber} ya fue entregado a ${carrier || 'el transportista'}. ${trackingCode ? 'Tracking: ' + trackingCode : ''} — Estampados DLV`;

  return {
    subject: `¡Tu pedido ${orderNumber} va en camino! 🚚`,
    html: shell({ title: `${orderNumber} en camino`, previewText: 'Entregamos tu pedido al transportista', contentHtml }),
    text,
  };
}

export function tplOrderDelivered({ orderNumber, customerName, proofUrl }) {
  const name = customerName?.split(' ')[0] || 'Hola';
  const proofInfo = proofUrl ? `
    <div style="text-align:center;margin:20px 0;">
      <a href="${proofUrl}" style="display:inline-block;padding:10px 20px;background:${BRAND.bgColor};color:${BRAND.textColor};border-radius:8px;text-decoration:none;font-size:13px;border:1px solid #e2e8f0;">Ver comprobante de entrega</a>
    </div>` : '';

  const contentHtml = `
    <h1 style="margin:0 0 8px 0;font-size:22px;font-weight:700;">¡Pedido entregado! ✨</h1>
    <p style="margin:0 0 20px 0;color:${BRAND.mutedColor};font-size:14px;">${escapeHtml(name)}, confirmamos que recibiste tu pedido.</p>
    ...
    <p style="margin:0;font-size:14px;color:${BRAND.textColor};">Esperamos que el resultado sea de tu total agrado. ¡Gracias por elegirnos!</p>`;

  const text = `${name}, tu pedido ${orderNumber} fue entregado. ¡Gracias por elegir Estampados DLV!`;

  return {
    subject: `¡Pedido ${orderNumber} entregado! ✨`,
    html: shell({ title: `${orderNumber} entregado`, previewText: 'Confirmamos la entrega de tu pedido', contentHtml }),
    text,
  };
}
...
// ---------------------------------------------------------------------------
// Concurso: ¡Ganaste! — Estampados DLV
// ---------------------------------------------------------------------------

export function tplContestWinner({ winnerName, prizeRank, prizeName, businessName }) {
  const contact = 'Sandra Vásquez';
  const phoneDisplay = '+56 9 5416 9052';
  const whatsappUrl = 'https://wa.me/56954169052';
  const prizeEmoji = { '1er lugar': '🥇', '2do lugar': '🥈', '3er lugar': '🥉' }[prizeRank] || '🏆';

  const contentHtml = `
    <div style="text-align:center;">
      <div style="font-size:48px;line-height:1;">${prizeEmoji}</div>
      <h1 style="margin:16px 0 8px 0;font-size:26px;color:${BRAND.colorDark};">¡Felicitaciones, ${escapeHtml(winnerName)}!</h1>
      <p style="margin:0 0 16px 0;color:${BRAND.mutedColor};font-size:15px;">
        Resultaste ganador/a del concurso de <strong style="color:${BRAND.textColor};">${escapeHtml(businessName)}</strong>
      </p>
    </div>

    <div style="background:${BRAND.colorLight};border-radius:12px;padding:20px;margin:20px 0;text-align:center;">
      <div style="font-size:13px;color:${BRAND.colorDark};font-weight:700;letter-spacing:1px;text-transform:uppercase;">${escapeHtml(prizeRank)}</div>
      <div style="font-size:22px;font-weight:700;color:${BRAND.textColor};margin-top:4px;">${escapeHtml(prizeName)}</div>
    </div>

    <p style="margin:16px 0;font-size:15px;color:${BRAND.textColor};">
      Para coordinar la entrega de tu premio, por favor <strong>contáctanos lo antes posible</strong> por WhatsApp con
      <strong>${contact}</strong>:
    </p>

    <div style="text-align:center;margin:24px 0;">
      <a href="${whatsappUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:14px 28px;background:${BRAND.color};color:#ffffff !important;font-weight:700;font-size:16px;text-decoration:none;border-radius:10px;">
        Escribir por WhatsApp: ${phoneDisplay}
      </a>
    </div>

    <p style="margin:16px 0;font-size:14px;color:${BRAND.mutedColor};">
      Ten tu teléfono a mano cuando nos escribas para verificar tu participación.
      El premio consiste en la prenda estampada con un diseño de tu elección
      (costo de envío a cargo del ganador/a).
    </p>

    <p style="margin:16px 0 0 0;font-size:14px;color:${BRAND.mutedColor};">
      ¡Gracias por compartir nuestra web y apoyar a Estampados DLV! 🎉
    </p>`;

  const text = `¡Felicitaciones, ${winnerName}! 🏆

Resultaste ganador/a del concurso de ${businessName}.

Premio: ${prizeRank} — ${prizeName}

Para coordinar la entrega de tu premio, contáctanos por WhatsApp con Sandra Vásquez al ${phoneDisplay}:
${whatsappUrl}

Ten tu teléfono a mano cuando nos escribas para verificar tu participación.
El premio consiste en la prenda estampada con un diseño de tu elección (costo de envío a cargo del ganador/a).

¡Gracias por compartir nuestra web y apoyar a Estampados DLV! 🎉`;

  return {
    subject: `🏆 ¡Ganaste en el concurso de ${businessName}!`,
    html: shell({
      title: `¡Ganaste en el concurso de ${businessName}!`,
      previewText: `Felicitaciones ${winnerName} — ${prizeRank}: ${prizeName}. Contáctanos por WhatsApp.`,
      contentHtml,
    }),
    text,
  };
...
// ---------------------------------------------------------------------------
// Confirmación de participación en concurso
// ---------------------------------------------------------------------------

export function tplContestParticipation({ email, contestTitle = 'Concurso Estampados DLV', endDateLabel = '12 de noviembre de 2026', businessName = 'Estampados DLV' }) {
  const siteUrl = 'https://estampadosdlv.com/concurso';
  const whatsappUrl = 'https://wa.me/56954169052';
  const phoneDisplay = '+56 9 5416 9052';
  const contact = 'Sandra Vásquez';
  ...
  const contentHtml = `
    <div style="text-align:center;">
      ...
      <h1 style="margin:16px 0 8px 0;font-size:26px;color:${BRAND.colorDark};">¡Tu participación está registrada!</h1>
      <p style="margin:0 0 16px 0;color:${BRAND.mutedColor};font-size:15px;">
        Ya estás participando en el concurso de <strong style="color:${BRAND.textColor};">${escapeHtml(businessName)}</strong>
      </p>
    </div>

    <div style="background:${BRAND.colorLight};border-radius:12px;padding:20px;margin:20px 0;text-align:center;">
      <div style="font-size:13px;color:${BRAND.colorDark};font-weight:700;letter-spacing:1px;text-transform:uppercase;">Sorteo termina el</div>
      <div style="font-size:22px;font-weight:700;color:${BRAND.textColor};margin-top:4px;">${escapeHtml(endDateLabel)}</div>
    </div>

    <p style="margin:16px 0;font-size:15px;color:${BRAND.textColor};">
      Este correo es tu confirmación de que quedaste inscrito/a en el sorteo.
      No necesitas hacer nada más: tus comprobantes quedaron registrados en nuestro sistema.
    </p>

    <p style="margin:16px 0;font-size:15px;color:${BRAND.textColor};"><strong>🎁 Premios en juego:</strong></p>
    <p style="margin:0 0 4px 0;font-size:14px;color:${BRAND.mutedColor};">🥇 1er lugar: Polerón personalizado estampado</p>
    <p style="margin:0 0 4px 0;font-size:14px;color:${BRAND.mutedColor};">🥈 2do lugar: Polera personalizada estampada</p>
    <p style="margin:0 0 16px 0;font-size:14px;color:${BRAND.mutedColor};">🥉 3er lugar: Gorra personalizada estampada</p>

    <p style="margin:16px 0;font-size:14px;color:${BRAND.mutedColor};">
      Los ganadores serán seleccionados aleatoriamente al terminar el sorteo y
      serán contactados por este correo y por teléfono.
    </p>

    <div style="text-align:center;margin:24px 0;">
      <a href="${siteUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:14px 28px;background:${BRAND.color};color:#ffffff !important;font-weight:700;font-size:16px;text-decoration:none;border-radius:10px;">
        Ver las bases del concurso
      </a>
    </div>

    <div style="border-top:1px solid #e2e8f0;margin:24px 0 0 0;padding-top:20px;">
      <p style="margin:0 0 10px 0;font-size:15px;color:${BRAND.textColor};font-weight:700;">🛍️ ¿Necesitas alguno de nuestros productos?</p>
      <p style="margin:0 0 10px 0;font-size:14px;color:${BRAND.mutedColor};">
        En <strong>${escapeHtml(businessName)}</strong> imprimimos en DTF Textil, DTF UV, poleras,
        polerones, gorras, tazones y mucho más. ¡Envío a todo Chile por solo $3.490!
      </p>
      <p style="margin:0 0 16px 0;font-size:14px;color:${BRAND.mutedColor};">
        No dudes en contactarnos para cotizar tu estampado:
      </p>
      <div style="text-align:center;">
        <a href="${whatsappUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:12px 24px;background:#25D366;color:#ffffff !important;font-weight:700;font-size:15px;text-decoration:none;border-radius:10px;">
          💬 Cotizar por WhatsApp: ${phoneDisplay}
        </a>
      </div>
    </div>

    <p style="margin:20px 0 0 0;font-size:14px;color:${BRAND.mutedColor};">
      Si tienes dudas sobre el concurso, escríbenos con ${contact}:
      <a href="${whatsappUrl}" target="_blank" rel="noopener noreferrer" style="color:${BRAND.color};font-weight:600;">${phoneDisplay}</a>
      <br>¡Mucha suerte! 🍀
    </p>`;

  const text = `🎉 ¡Tu participación está registrada!

Ya estás participando en el concurso de ${businessName}.

Sorteo termina el ${endDateLabel}.

Este correo es tu confirmación de que quedaste inscrito/a en el sorteo.
No necesitas hacer nada más: tus comprobantes quedaron registrados en nuestro sistema.

🎁 Premios en juego:
🥇 1er lugar: Polerón personalizado estampado
🥈 2do lugar: Polera personalizada estampada
🥉 3er lugar: Gorra personalizada estampada

Los ganadores serán seleccionados aleatoriamente al terminar el sorteo y
serán contactados por este correo y por teléfono.

🛍️ ¿Necesitas alguno de nuestros productos?
En ${businessName} imprimimos en DTF Textil, DTF UV, poleras, polerones,
gorras, tazones y mucho más. ¡Envío a todo Chile por solo $3.490!
No dudes en contactarnos para cotizar tu estampado:
Ver catálogo: https://estampadosdlv.com/tienda
Cotizar por WhatsApp: ${phoneDisplay} — ${whatsappUrl}

Ver las bases del concurso: ${siteUrl}
...
¡Mucha suerte! 🍀`;
  ...
  return {
    subject: `🎉 ¡Ya estás participando en el concurso de ${businessName}!`,
    html: shell({
      title: `¡Tu participación está registrada!`,
      previewText: `Tu participación en el concurso queda confirmada. Sorteo termina el ${endDateLabel}. ¡Mucha suerte!`,
      contentHtml,
    }),
    text,
  };
