...
export async function getPublicProductBySlug(slug) {
  // Detalle de producto: necesita TODAS las variantes (precios por talla/color
  // y mapeo de stock del cliente). La lista de tarjetas sí usa $slice: 1.
  try {
    const db = await getDb();
    const doc = await db.collection(COLLECTIONS.PRODUCTS).findOne({ slug, active: { $ne: false } });
    return doc ? strip(doc) : null;
  } catch (e) {
    console.error('[store-data] getPublicProductBySlug failed:', e.message);
    return null;
  }
}
