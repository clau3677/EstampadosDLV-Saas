...
import { consumeStockForPosSale } from './orders';
...
async function handleSalesCreate(ctx) {
  ...
  // Upsert cliente (sólo si hay datos reales — evita crear "Cliente Web" genéricos)
  try {
    if (order.customerSnapshot?.email || order.customerSnapshot?.phone || order.customerSnapshot?.rut) {
      const customerDoc = await upsertCustomerFromSnapshot(db, order.customerSnapshot, {
        source: SALES_CHANNEL.POS,
        channels: [SALES_CHANNEL.POS],
      });
      if (customerDoc?.id) {
        await db.collection(COLLECTIONS.ORDERS).updateOne({ id: orderId }, { $set: { customerId: customerDoc.id } });
      }
    }
  } catch (e) {
    console.warn('[pos] upsertCustomerFromSnapshot failed:', e?.message);
  }
  ...
  try {
    await consumeStockForPosSale(db, resolved, orderId, orderNumber, session.operatorId);
  } catch (stockError) {
    ...
    return err(stockError.message || 'No se pudo consumir el stock', 409);
  }
  ...
  // Notificación WhatsApp best-effort (solo si el cliente dejó teléfono en POS)
  notifyOrderConfirmation({ order, items: orderItems })
    .catch((e) => console.warn('[wa] pos confirmation dispatch failed:', e.message));

  // Notificación Email best-effort (solo si el cliente dejó email en POS)
  notifyOrderConfirmationByEmail({ order, items: orderItems })
    .catch((e) => console.warn('[email] pos confirmation dispatch failed:', e.message));
