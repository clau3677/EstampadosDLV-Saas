...
const QUEUE_COLL = COLLECTIONS.NOTIFICATION_JOBS;
...
async function processJob(db, job) {
  ...
  try {
    ...
    } else if (['no_email', 'invalid_email', 'no_phone', 'invalid_phone', 'smtp_not_configured'].includes(result.reason)) {
      await db.collection(QUEUE_COLL).updateOne({ id: job.id }, {
        $set: { status: 'skipped', attempts: job.attempts + 1, error: result.reason, updatedAt: new Date() },
        $unset: { processingAt: '' },
      });
    } else {
...
// En el VPS persistente, el worker revisa la cola cada minuto. `unref` evita
// mantener vivo un proceso durante build/tests si no hay un servidor activo.
if (typeof window === 'undefined' && !globalThis.__dlvNotificationQueueTimer) {
  const timer = setInterval(() => {
    processQueue().catch(error => console.error('[queue] periodic worker error:', error.message));
  }, 60 * 1000);
  timer.unref?.();
  globalThis.__dlvNotificationQueueTimer = timer;
}
