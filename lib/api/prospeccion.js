...
import { listCampaigns, getCampaign, createCampaign, updateCampaign, pauseCampaign, resumeCampaign, completeCampaign, cancelCampaign, CHANNEL_LABELS } from '@/lib/prospeccion/campaigns';
...
async function handleMessagesGet(request) {
  return withAdmin(request, async () => {
    ...
    if (qs.get('preview') === '1' && qs.get('leadId')) {
      ...
      if (!lead) return err('Prospecto no encontrado', 404);
      const channel = qs.get('channel') || 'email';
      const preview = previewMessage(lead, lead.category || 'otros', channel.startsWith('whatsapp') ? 'whatsapp' : 'email');
