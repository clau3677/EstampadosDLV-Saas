...
import { listCampaigns, getCampaign, createCampaign, updateCampaign, pauseCampaign, resumeCampaign, completeCampaign, cancelCampaign, deleteCampaign, CHANNEL_LABELS } from '@/lib/prospeccion/campaigns';
import { runDiscoveryForCampaign, runDiscoveryByFilter } from '@/lib/prospeccion/discovery';
import { listLeads, getLead, updateLeadState, listLeadStats, bulkApproveLeads } from '@/lib/prospeccion/leads';
import { enrichLeadsWithEmails } from '@/lib/prospeccion/emailfinder';
import { isSimulationMode, previewMessage, approveMessageBatch, sendTestMessage } from '@/lib/prospeccion/messages';
import { sendOne, processJobBatch, LIMITS } from '@/lib/prospeccion/mailer';
import { isWhatsappConnected, whatsappUserInfo } from '@/lib/prospeccion/wasender';
import { listSuppressions, addSuppression, removeSuppression } from '@/lib/prospeccion/suppression';
import { listAuditEvents, logAudit, AUDIT_ACTIONS } from '@/lib/prospeccion/audit';
import { listCategories, listMessageTemplates } from '@/lib/prospeccion/templates';
import { isScraperEnabled } from '@/lib/prospeccion/discovery/scraper';
import { isGooglePlacesEnabled } from '@/lib/prospeccion/discovery/googlePlaces';
...
export default async function handleProspection({ request, method, route, db }) {
  ...
  const routes = [
    ...
    { pattern: '/prospeccion/campaigns/:id', get: handleCampaignGet, patch: handleCampaignPatch, delete: handleCampaignDelete },
    { pattern: '/prospeccion/campaigns/:id/leads', get: handleCampaignLeadsGet },
    // Descubrimiento
    { pattern: '/prospeccion/discovery', post: handleDiscoveryPost },
    { pattern: '/prospeccion/discovery-direct', post: handleDiscoveryDirectPost },
    // Leads
    { pattern: '/prospeccion/leads', get: handleLeadsGet },
    { pattern: '/prospeccion/leads/approve-all', post: handleLeadsBulkApprove },
    { pattern: '/prospeccion/leads/enrich-emails', post: handleLeadsEnrichEmails },
    ...
    { pattern: '/prospeccion/templates', get: handleTemplatesGet },
    // Envío real (jobs)
...
async function handleCampaignPatch(request, params) {
  return withAdmin(request, async (user) => {
    ...
    try {
      ...
      else if (body.action === 'resume' || body.action === 'activate') result = await resumeCampaign(params.id, user.id, user.name);
      else if (body.action === 'complete') result = await completeCampaign(params.id, user.id, user.name);
      else if (body.action === 'cancel') result = await cancelCampaign(params.id, user.id, user.name);
...
async function handleCampaignDelete(request, params) {
  return withAdmin(request, async (user) => {
    try {
      const result = await deleteCampaign(params.id, user.id, user.name);
      return json(strip(result));
    } catch (e) {
      return err(e.message, 400);
    }
...
// Ejecuta descubrimiento directo por rubros/comunas SIN campaña
async function handleDiscoveryDirectPost(request) {
  return withAdmin(request, async (user) => {
    const body = await readBody(request);
    try {
      const outcome = await runDiscoveryByFilter({
        categories: body.categories || null,
        communes: body.communes || null,
        provider: body.provider || null,
        // build123: por defecto cobertura masiva (20.000) en vez de 500
        limit: body.limit || 20000,
        actorId: user.id,
      });
      return json({
        saved: outcome.saved,
        skipped: outcome.skipped,
        errors: outcome.result?.errors || [],
        discovered: outcome.result?.discovered?.slice(0, 20) || [],
      });
    } catch (e) {
      return err(e.message, 400);
    }
...
async function handleLeadsCsv(request) {
  return withAdmin(request, async () => {
    const data = await listLeads({
      ...
      sortBy: qsRef(request).get('sortBy') || undefined,
      phoneType: qsRef(request).get('phoneType') || undefined,
    });
    const rows = [
...
async function handleLeadsBulkApprove(request) {
  return withAdmin(request, async (user) => {
    const body = await readBody(request);
    try {
      ...
      await logAudit({
        ...
        entityType: 'leads',
        entityId: 'bulk',
        details: { bulkApprove: true, filters: body, ...result },
      });
...
// Enriquecimiento de correos empresariales desde el sitio web oficial (build117)
async function handleLeadsEnrichEmails(request) {
  return withAdmin(request, async (user) => {
    const body = await readBody(request);
    try {
      const result = await enrichLeadsWithEmails({
        category: body.category,
        commune: body.commune,
        minScore: body.minScore ?? null,
        q: body.q,
        phoneType: body.phoneType,
        sortBy: body.sortBy,
        limit: Math.min(200, body.limit ?? 100),
      });
      await logAudit({
...
async function handleMessagesGet(request) {
  return withAdmin(request, async () => {
    ...
    if (qs.get('preview') === '1' && qs.get('leadId')) {
      ...
      if (!lead) return err('Prospecto no encontrado', 404);
      const channel = qs.get('channel')?.startsWith('whatsapp') ? 'whatsapp' : 'email';
      let templateId = qs.get('templateId') || null;
      ...
      if (!templateId && campaignId) {
        ...
        templateId = channel === 'whatsapp' ? campaign?.whatsappTemplateId : campaign?.emailTemplateId;
      }
      const preview = previewMessage(lead, lead.category || 'otros', channel, { templateId });
    ...
    // Enriquecer con datos del negocio para mostrar en el panel (nombre, comuna, rubro)
    const leadIds = [...new Set(items.map(m => m.leadId).filter(Boolean))];
    let leadByName = {};
    if (leadIds.length) {
      const leads = await coll(COLLECTIONS.PRO_LEADS);
      const leadDocs = await leads.find({ id: { $in: leadIds } }).toArray();
      leadByName = Object.fromEntries(leadDocs.map(l => [l.id, l]));
    }
    return json({
      items: items.map(m => {
        const lead = leadByName[m.leadId];
        return strip({
          ...m,
          leadName: lead ? (lead.name || null) : null,
          leadCommune: lead ? (lead.commune || null) : null,
          leadCategory: lead ? (lead.category || null) : null,
        });
      }),
      page, pageSize, total,
    });
...
async function handleTemplatesGet(request) {
  return withAdmin(request, async () => {
    const channel = qsRef(request).get('channel');
    return json({ items: listMessageTemplates(channel || null) });
  });
}
...
async function handleMessagesTest(request) {
  return withAdmin(request, async (user) => {
    const body = await readBody(request);
    const messages = await coll(COLLECTIONS.PRO_MESSAGES);
    // Prueba por WhatsApp: { toPhone, category }
    if (body.toPhone) {
      if (!isWhatsappConnected()) return err('WhatsApp no está conectado. Vincula la sesión primero.', 400);
      const { renderTemplate, SERVICE_BY_CATEGORY } = await import('@/lib/prospeccion/templates');
      const category = body.category || 'restaurantes';
      const preview = renderTemplate('whatsapp', category, { name: 'Negocio de prueba', commune: 'Quilpué', category, phone: body.toPhone });
      const msg = {
        id: `test-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
        campaignId: null, campaignLeadId: null, leadId: null,
        channel: 'whatsapp', body: preview.body,
        recipient: String(body.toPhone).replace(/\D/g, ''),
        test: true, status: 'pendiente', simulationMode: false,
        createdAt: new Date(),
      };
      await messages.insertOne(msg);
      const result = await sendOne({
        messageId: msg.id, campaignId: null, recipient: msg.recipient,
        body: msg.body, test: true, actorId: user.id, actorName: user.name,
        channel: 'whatsapp',
      });
...
async function handleConfigGet() {
  return withAdmin(arguments[0], async () => json({
    ...
    providerConfigured: Boolean(process.env.SMTP_APP_PASSWORD || process.env.SMTP_PASS || process.env.RESEND_API_KEY),
    from: process.env.PROSPECTION_FROM || 'Sandra Vásquez <estampadosdlv@gmail.com>',
    ...
    limits: { dailyMax: LIMITS.dailyMax, dailyBurst: LIMITS.dailyBurst, startHour: LIMITS.startHour, endHour: LIMITS.endHour },
    whatsapp: { enabled: true, ...whatsappStatus() },
