...
export default async function handleMarketing(ctx) {
  ...
  if (route === '/marketing/contest/participate' && method === 'POST') {
    ...
    if (!name || !email || !phone) return err('Nombre, email y teléfono son obligatorios');
    if (email.length > 100 || name.length > 100 || phone.length > 20) return err('Campos demasiado largos');
  ...
  if (route === '/marketing/contest/create' && method === 'POST') {
    const cs = request.headers.get('x-cron-secret');
    if (!(cs && process.env.MARKETING_CRON_SECRET && cs === process.env.MARKETING_CRON_SECRET) && !requireAdmin(request)) return err('No autorizado', 401);
    const body = await request.json();
