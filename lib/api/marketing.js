...
export default async function handleMarketing(ctx) {
  ...
  if (route === '/marketing/auto/schedule' && method === 'POST') {
    ...
    for (const promo of promoSlots) {
      ...
      // Seleccionar imagen del producto (rotar entre imágenes disponibles)
      const promoImage = promoProduct.images?.[0] || '';
      const promoPost = {
        id: promoPostId,
        ...
        imageUrl: promoImage ? await composePostImage({
          ...
          productName: promoName,
          priceClp: promoPrice,
          // FIX: incluir productId para que no se sobrescriba cada día
    ...
    // --- POSTS DE VIDEO PROMOCIONAL (3 por día, con rotación y audio real) ---
    // Videos con VOZ EN OFF real (audio verificado) - rotación diaria
    // Horarios: 8:00, 14:00, 19:00
    const allVideos = [
      {
        videoUrl: '/videos/video-plotter.mp4',
        title: 'Plotter DTF en acción',
        caption: `¡Nuestra tecnología DTF en acción! 🖨️✨\n\nCon el plotter Prestige R2 Pro imprimimos diseños de alta calidad con colores vibrantes y durabilidad profesional.\n\n👉 Conoce más: https://estampadosdlv.com\n\n📍 Envío a todo Chile: $3.490 · 2-5 días hábiles\n📞 WhatsApp: +56 9 5416 9052\n\n#dtf #dtftextil #dtfuv #estampados #estampadosdlv #quilpue #valparaiso #emprendedoreschile #impresion #personalizacion #pymechile #modachilena`,
        occasion: 'Video Promocional - Plotter DTF',
      },
      {
        videoUrl: '/videos/video6-mockup-promo.mp4',
        title: 'Editor de Mockups',
        caption: `¡Crea tu mockup GRATIS! 🎨👕\n\nSube tu diseño y pruébalo sobre nuestras poleras, gorras y más. Sin necesidad de comprar para ver cómo quedará.\n\n👉 Pruébalo aquí: https://estampadosdlv.com/mockup\n\n📍 Envío a todo Chile: $3.490 · 2-5 días hábiles\n📞 WhatsApp: +56 9 5416 9052\n\n#mockup #diseñografico #dtf #estampados #estampadosdlv #personalizacion #poleras #gorras #gratis #quilpue #chile`,
        occasion: 'Video Promocional - Editor Mockups',
      },
      {
        videoUrl: '/videos/video7-tienda-promo.mp4',
        title: 'Tienda Online',
        caption: `¡Compra online con envío a todo Chile! 🛍️🇨🇱\n\nPoleras, polerones, gorras, tazas y más. Estampados DTF de alta calidad con la mejor presentación.\n\n👉 Explora el catálogo: https://estampadosdlv.com/tienda\n\n📍 Envío a todo Chile: $3.490 · 2-5 días hábiles\n📞 WhatsApp: +56 9 5416 9052\n\n#estampadosdlv #dtf #tiendaonline #poleras #gorras #polerones #personalizacion #quilpue #valparaiso #chile`,
        occasion: 'Video Promocional - Tienda',
      },
      {
        videoUrl: '/videos/video8-gang-sheet-tutorial.mp4',
        title: 'Gang Sheet Builder',
        caption: `¡Arma tu pañol DTF y ahorra! 🖨️💰\n\nCrea tu gang sheet gratis con nuestra herramienta. Sube tus diseños, organízalos y nosotros los imprimimos.\n\n👉 Prueba el Gang Sheet Builder: https://estampadosdlv.com/gang-sheet\n\n📍 Envío a todo Chile: $3.490 · 2-5 días hábiles\n📞 WhatsApp: +56 9 5416 9052\n\n#gangsheet #dtf #impresion #estampados #estampadosdlv #ahorro #personalizacion #emprendedores #quilpue #chile`,
        occasion: 'Video Promocional - Gang Sheet Builder',
      },
    ];

    // ROTACIÓN: Seleccionar 3 videos diferentes cada día, sin repetir hasta usar todos
    const videoUsedKey = `video-cycle-${Math.floor(now.getTime() / (24 * 3600 * 1000))}`;
    const videoCycleData = await db.collection('marketing_video_cycle').findOne({ key: videoUsedKey });
    let videoIndex = videoCycleData?.index || 0;
    
    // Seleccionar 3 videos consecutivos del pool (rotando)
    const todayVideos = [];
    for (let i = 0; i < 3; i++) {
      todayVideos.push(allVideos[(videoIndex + i) % allVideos.length]);
    }
    // Actualizar índice para mañana
    await db.collection('marketing_video_cycle').updateOne(
      { key: videoUsedKey },
      { $set: { key: videoUsedKey, index: (videoIndex + 3) % allVideos.length, date: new Date() } },
      { upsert: true }
    );

    // Horarios de video: 8:00, 14:00, 19:00
    const videoHours = [8, 14, 19];

    // Programar los 3 videos en sus horarios respectivos
    for (let vi = 0; vi < todayVideos.length; vi++) {
      const selectedVideo = todayVideos[vi];
      ...
      const videoPost = {
        id: videoPostId,
        productId: 'video-promo',
        productSlug: 'tienda',
        productName: selectedVideo.title,
        category: 'video_promo',
        text: selectedVideo.caption,
        caption: selectedVideo.caption,
        fullCaption: selectedVideo.caption,
        link: 'https://estampadosdlv.com',
        hashtags: '#dtf #estampados #estampadosdlv #quilpue #valparaiso #chile',
        altText: selectedVideo.occasion,
        tone: 'video',
        occasion: selectedVideo.occasion,
        platforms: ['facebook', 'instagram'],
        status: 'scheduled',
        autoScheduled: true,
        isVideo: true,
        videoUrl: `${BASE}${selectedVideo.videoUrl}`,
        imageUrl: '', // No tiene imagen, es video
        createdAt: new Date(),
        updatedAt: new Date(),
      ...
      results.push({ ok: true, postId: videoPostId, productName: selectedVideo.title, occasion: selectedVideo.occasion, publishAt: videoPublishAt, video: true });
    }
