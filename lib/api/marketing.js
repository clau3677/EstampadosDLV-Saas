  ...
  if (route === '/marketing/auto/schedule' && method === 'POST') {
    ...
    for (const cat of categories) {
      const products = await db.collection(COLLECTIONS.PRODUCTS)
        ...
        .limit(50)
        .toArray();
      // Filtrar los ya publicados en este ciclo
      ...
      try {
        ...
        const post = {
          ...
          imageUrl: await composePostImage({
            ...
            fileStem: `auto-${postId.slice(0, 8)}`,
          }).then(r => r.relativeUrl).catch(() => product.images?.[0] || ''),
        };
    ...
    const totalActive = await db.collection(COLLECTIONS.PRODUCTS).countDocuments({ active: { $ne: false } });
    const publishedCount = await db.collection('marketing_published_products').countDocuments({});
    if (publishedCount >= totalActive && results.some(r => r.ok)) {
      // Ciclo completado - resetear para empezar de nuevo
    ...
    for (const promo of promoSlots) {
      ...
      const promoProduct = promoProducts[Math.floor(Math.random() * Math.min(promoProducts.length, 5))];
    ...
    // --- POSTS DE VIDEO PROMOCIONAL (1 por día, cada 2 días rotando) ---
    // Videos pregenerados de alta calidad: plotter, poleras, gorras, negocio
    // Se alternan entre 4 videos cada día para mantener contenido fresco
    const videoPosts = [
      {
        hour: 12, // Se publica a las 12:00 los días pares
        videoUrl: '/videos/video-plotter-v2.mp4',
      ...
      {
        hour: 12,
        videoUrl: '/videos/video-poleras-v2.mp4',
        caption: `¡Poleras estampadas con diseño profesional! 👕🎨\n\nColores vibrantes, calidad DTF y diseños que destacan. Desde 1 unidad hasta grandes volúmenes.\n\n👉 Compra aquí: https://estampadosdlv.com/tienda\n\n📍 Envío a todo Chile: $3.490 · 2-5 días hábiles\n📞 WhatsApp: +56 9 5416 9052\n\n#poleras #poleraspersonalizadas #dtf #estampados #estampadosdlv #quilpue #valparaiso #modachilena #emprendedoreschile #pymechile`,
        tone: 'video',
        occasion: 'Video Promocional - Poleras',
      },
      {
        hour: 12,
        videoUrl: '/videos/video-gorras-v2.mp4',
        caption: `¡Gorras personalizadas para tu marca! 🧢\n\nBordado o impresión DTF en gorras de alta calidad. Ideal para empresas, eventos y ventas.\n\n👉 Compra aquí: https://estampadosdlv.com/tienda\n\n📍 Envío a todo Chile: $3.490 · 2-5 días hábiles\n📞 WhatsApp: +56 9 5416 9052\n\n#gorras #gorraspersonalizadas #dtf #estampados #estampadosdlv #quilpue #valparaiso #empresas #promocional #pymechile`,
        tone: 'video',
        occasion: 'Video Promocional - Gorras',
      },
      {
        hour: 12,
        videoUrl: '/videos/video-negocio-v2.mp4',
        caption: `¡Todo lo que necesitas en un solo lugar! 🛍️\n\nPoleras, polerones, gorras, tazas y más. Estampados personalizados con la mejor calidad DTF.\n\n👉 Explora nuestro catálogo: https://estampadosdlv.com/tienda\n\n📍 Envío a todo Chile: $3.490 · 2-5 días hábiles\n📞 WhatsApp: +56 9 5416 9052\n\n#estampadosdlv #dtf #dtftextil #dtfuv #poleras #gorras #polerones #tazas #personalizacion #quilpue #valparaiso #chile`,
        tone: 'video',
        occasion: 'Video Promocional - Negocio',
      },
    ];

    // Rotar entre los 4 videos: usar el día del mes para decidir cuál
    const dayOfMonth = now.getDate();
    const videoIndex = (dayOfMonth - 1) % videoPosts.length;
    const selectedVideo = videoPosts[videoIndex];
    const videoPostId = `video-${Date.now()}`;
    const videoPublishAt = new Date(now);
    videoPublishAt.setHours(selectedVideo.hour, 0, 0, 0);
    ...
    const videoPost = {
      ...
      productSlug: 'tienda',
      productName: 'Video Promocional Estampados DLV',
    ...
    results.push({ ok: true, postId: videoPostId, productName: 'Video Promocional', occasion: selectedVideo.occasion, publishAt: videoPublishAt, video: true });
