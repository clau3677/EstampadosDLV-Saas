    ...
    if (action === 'update') {
      ...
      if (ctB.includes('multipart/form-data')) {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          const publicDirB = path.join(process.cwd(), '..', 'public', relDirB);
        ...
        try { const bB = await request.json(); titleB = bB.title; startDateB = bB.startDate; endDateB = bB.endDate; prizesB = Array.isArray(bB.prizes) ? bB.prizes : []; } catch { return err('Cuerpo inválido', 400); }
      }
