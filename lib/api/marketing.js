...
export default async function handleMarketing(ctx) {
  ...
  if (route === '/marketing/contest/upload-proof' && method === 'POST') {
    ...
    // En producción (output: standalone) process.cwd() es la carpeta del
    // proyecto, pero el public servido por nginx (alias /uploads/) es
    // <proyecto>/../public (=/var/www/estampadosdlv/public en producción).
    // Guardamos en AMBAS ubicaciones con rutas absolutas para asegurar que
    // siempre se vea:
    const publicDirCwd = path.resolve(process.cwd(), 'public', rel);
    const publicDirNginx = path.resolve(process.cwd(), '..', 'public', rel);
    try {
      await fs.mkdir(publicDirNginx, { recursive: true });
      await fs.writeFile(path.join(publicDirNginx, fileName), fileBuf);
    } catch { /* noop: carpeta padre no existe fuera de producción */ }
    try {
