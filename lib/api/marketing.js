...
export default async function handleMarketing(ctx) {
  ...
  if (route === '/marketing/auto/schedule' && method === 'POST') {
    ...
    // --- POSTS DE VIDEO PROMOCIONAL (4 por día) ---
    // 4 videos promocionales diarios con voz en off y fotos reales de productos
    // Horarios: 7:00, 13:00, 16:00, 20:00
    const videoPosts = [
      {
        hour: 7,
    ...
    // Programar los 4 videos en sus horarios respectivos
    for (const selectedVideo of videoPosts) {
      const videoPostId = `video-${Date.now()}-${selectedVideo.hour}`;
