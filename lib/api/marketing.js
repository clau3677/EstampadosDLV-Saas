...
const PUBLIC_ROOT = process.env.PUBLIC_ROOT || '/var/www/estampadosdlv/public';

async function assertPublicMediaAvailable(mediaValue) {
  if (!mediaValue) throw new Error('Post sin imagen o video publicable');
  let pathname = mediaValue;
  ...
  if (!pathname.startsWith('/uploads/') && !pathname.startsWith('/videos/')) return;
  const relative = pathname.replace(/^\/+/, '');
  const candidate = path.resolve(PUBLIC_ROOT, relative);
  const root = path.resolve(PUBLIC_ROOT);
  if (!candidate.startsWith(`${root}${path.sep}`)) throw new Error('Ruta multimedia fuera de la carpeta pública');
...
async function publishPost(db, post, account) {
  ...
  const publishToken = pageToken || directToken;
  if (!publishToken) throw new Error('Cuenta Meta sin page token — reconecta la cuenta');

  const results = { facebook: null, instagram: null };
  const errors = [];
  const requestedPlatforms = Array.isArray(post.platforms) && post.platforms.length
    ? post.platforms.filter((platform) => platform === 'facebook' || platform === 'instagram')
    : ['facebook', 'instagram'];

  // Si el post tiene videoUrl, publicar como video; si no, como foto.
  // Meta descarga el archivo desde Internet; validar antes evita enviar URLs 404.
  const isVideo = !!post.videoUrl;
  const mediaPath = isVideo ? post.videoUrl : post.imageUrl;
  await assertPublicMediaAvailable(mediaPath);
  const mediaUrl = absUrl(mediaPath);
  ...
  if (requestedPlatforms.includes('instagram') && !account.igUserId) {
    errors.push('instagram: la cuenta de Instagram no está conectada o no tiene igUserId');
  } else if (requestedPlatforms.includes('instagram')) {
    try {
      const limit = await getInstagramPublishingLimit(account.igUserId, publishToken);
  ...
  const missingPlatforms = requestedPlatforms.filter((platform) => !results[platform]);
  if (missingPlatforms.length) {
    for (const platform of missingPlatforms) {
      const label = platform === 'facebook' ? 'Facebook' : 'Instagram';
      if (!errors.some((error) => error.toLowerCase().startsWith(`${platform}:`))) {
        errors.push(`${platform}: Meta no devolvió un identificador de publicación para ${label}`);
      }
    }
  }
  const fullyPublished = requestedPlatforms.length > 0 && missingPlatforms.length === 0;
  const now = new Date();
  await db.collection(COLLECTIONS.MARKETING_POSTS).updateOne(
    { id: post.id },
    {
      $set: {
        status: fullyPublished ? 'published' : 'failed',
        publishedAt: fullyPublished ? now : null,
  ...
  return { ok: fullyPublished, results, errors, requestedPlatforms, missingPlatforms };
}
...
export default async function handleMarketing(ctx) {
  ...
  if (route === '/marketing/dispatch' && method === 'POST') {
    ...
    try {
      ...
      for (const sp of stuckPosts) {
        if (sp.fbPostId && sp.igMediaId) {
          ...
          // Publicación parcial: Facebook sí respondió, Instagram no. No informar éxito completo.
          await db.collection(COLLECTIONS.MARKETING_POSTS).updateOne(
            { id: sp.id },
            {
              $set: {
                status: 'failed',
                publishErrors: [...(sp.publishErrors || []), 'instagram: publicación pendiente tras recuperación automática'],
                updatedAt: new Date(),
              },
            }
          );
    ...
    const directToken = process.env.META_PAGE_ACCESS_TOKEN;
    const directPageId = process.env.META_PAGE_ID;
    // El dispatch debe funcionar tanto con la cuenta OAuth almacenada como
    // con las credenciales directas configuradas en el entorno.
    if (account?.pageToken || (directToken && directPageId)) {
      const now = new Date();
      ...
      const duePosts = await db.collection(COLLECTIONS.MARKETING_POSTS)
        .find({
          status: 'scheduled',
          scheduledAt: { $lte: now },
          $or: [{ fbPostId: { $exists: false } }, { fbPostId: null }, { fbPostId: '' }],
        })
      ...
      const igRetryPosts = await db.collection(COLLECTIONS.MARKETING_POSTS)
        .find({
          ...
          igMediaId: null,
          publishErrors: { $elemMatch: { $regex: '^instagram' } },
        })
      ...
      for (const post of igRetryPosts) {
        ...
        try {
          ...
          // Los reels usan el flujo de contenedor; las fotos usan el helper
          // de publicación fotográfica para no dejar fallos parciales sin retry.
          const { pageToken } = accountTokens(account);
          const retryToken = pageToken || directToken;
          const retryPageId = directPageId || account.pageId;
          const mediaUrl = absUrl(post.videoUrl || post.imageUrl);
          if (!retryToken || !retryPageId) throw new Error('Faltan credenciales Meta para reintentar Instagram');
          const igData = await graphFetch(`/${retryPageId}`, {
            token: retryToken,
            params: { fields: 'instagram_business_account{id,username}' },
          });
          const igUserId = igData.instagram_business_account?.id;
          if (igUserId && post.videoUrl) {
            const container = await graphFetch(`/${igUserId}/media`, {
              method: 'POST',
              token: retryToken,
              params: {
            ...
            await db.collection(COLLECTIONS.MARKETING_POSTS).updateOne(
              { id: post.id },
              { $set: { status: 'published', igMediaId: published.id, publishErrors: [], updatedAt: new Date() } }
            );
            summary.publishedPosts += 1;
          } else if (account.igUserId && !post.videoUrl) {
            const published = await publishInstagramPhoto({
              igUserId: account.igUserId,
              pageToken: retryToken,
              imageUrl: absUrl(post.imageUrl),
              caption: post.fullCaption || post.caption || '',
              altText: post.altText,
            });
            await db.collection(COLLECTIONS.MARKETING_POSTS).updateOne(
              { id: post.id },
              { $set: { status: 'published', igMediaId: published.mediaId, publishErrors: [], updatedAt: new Date() } }
            );
            summary.publishedPosts += 1;
          }
        } catch (e) {
      ...
      try {
        ...
        if (isGeneratorConfigured()) {
          try {
            ...
            postContent = {
              caption: `¡Nuevo producto en Estampados DLV! 🎉\n\n${product.name}\n✨ Cotiza según tu cantidad y personalización.\n\n👉 Conoce el producto y cotiza aquí: https://estampadosdlv.com/producto/${product.slug}`,
        ...
        const post = {
          ...
          imageUrl: await composePostImage({
            ...
            benefit: product.marketingBenefit || product.shortDescription || '',
            badge: 'NUEVO · COLECCIÓN DLV',
    ...
    for (const promo of promoSlots) {
      ...
      const promoPost = {
        ...
        imageUrl: promoImage ? await composePostImage({
          ...
          productName: promoName,
          category: promo.category,
          campaign: `Promoción ${promo.label}`,
          benefit: promo.category === 'dtf_uv' ? 'Colores intensos · Acabado premium · Alta adherencia' : 'Colores vibrantes · Durabilidad · Calidad profesional',
          badge: 'SERVICIO DLV',
  ...
  if (route === '/marketing/posts/generate' && method === 'POST') {
    const { productId, tone, occasion, platforms, scheduledAt, caption: customCaption, hashtags: customHashtags, headline, subheadline, benefit, cta, color, backgroundColor, secondaryColor, productBackdropColor, removeBackground, sourceImage } = await request.json();
    ...
    const { relativeUrl } = await composePostImage({
      sourceImage: sourceImage || product.images[0],
       productName: product.name,
      category: product.category,
      campaign: occasion,
      benefit: benefit || product.marketingBenefit || product.shortDescription || '',
      badge: 'ESTAMPADOSDLV.COM',
      color,
      backgroundColor,
      secondaryColor,
      productBackdropColor,
      cta,
      titleLines: [headline || product.name, subheadline || 'PERSONALIZA TU IDEA'],
      removeBackground: removeBackground !== false,
      fileStem: `post-${postId.slice(0, 8)}`,
    });

    const now = new Date();
    const admin = requireAdmin(request) || { id: 'system', email: 'system@estampadosdlv.com' };
    const post = {
      id: postId,
      ...
      caption: customCaption ? String(customCaption).slice(0, 2000) : content.caption,
      hashtags: Array.isArray(customHashtags) && customHashtags.length ? customHashtags.slice(0, 12) : content.hashtags,
      altText: content.altText,
      fullCaption: `${customCaption ? String(customCaption).slice(0, 2000) : content.caption}\n\n${(Array.isArray(customHashtags) && customHashtags.length ? customHashtags : content.hashtags).join(' ')}`,
      imageUrl: relativeUrl,
      design: { headline, subheadline, benefit, cta, color, backgroundColor, secondaryColor, productBackdropColor, removeBackground: removeBackground !== false, sourceImage: sourceImage || null },
  ...
  if (route === '/marketing/posts' && method === 'PATCH') {
    const { id, caption, hashtags, scheduledAt, platforms, status, design } = await request.json();
    ...
    if (design && typeof design === 'object') {
      const product = await db.collection(COLLECTIONS.PRODUCTS).findOne({ id: post.productId });
      if (!product?.images?.length) return err('El producto del borrador ya no tiene una imagen disponible', 400);
      const regenerated = await composePostImage({
        sourceImage: design.sourceImage || product.images[0],
        productName: product.name || post.productName,
        category: product.category,
        campaign: design.campaign || post.occasion || 'Editor creativo DLV',
        benefit: design.benefit || product.marketingBenefit || product.shortDescription || '',
        badge: 'ESTAMPADOSDLV.COM',
        color: design.color,
        backgroundColor: design.backgroundColor,
        secondaryColor: design.secondaryColor,
        productBackdropColor: design.productBackdropColor,
        cta: design.cta,
        titleLines: [design.headline || product.name, design.subheadline || 'PERSONALIZA TU IDEA'],
        removeBackground: design.removeBackground !== false,
        fileStem: `post-${post.id.slice(0, 8)}-${Date.now()}`,
      });
      set.imageUrl = regenerated.relativeUrl;
      set.design = design;
    }
