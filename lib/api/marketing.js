...
export default async function handleMarketing(ctx) {
  ...
  if (route === '/marketing/contest' && method === 'GET') {
    ...
    return json({
      contest: {
        ...
        prize: contest.prize, prizes: contest.prizes || null, ogImage: contest.ogImage || '/uploads/contest/og-concurso.jpg',
  ...
  if (route === '/marketing/contest/drawnames' && method === 'GET') {
    ...
    const prizesLabels = Array.isArray(contest.prizes)
      ? contest.prizes.map((p, i) => ({ rank: ['1er Lugar', '2do Lugar', '3er Lugar'][i], prize: p?.label || ['Polerón personalizado', 'Polera personalizada', 'Gorra personalizada'][i], image: p?.image || ['/uploads/contest/premio-poleron.png', '/uploads/contest/premio-polera.png', '/uploads/contest/premio-gorra.png'][i] }))
      : null;
  ...
  if (route === '/marketing/contest/update' && method === 'POST') {
    ...
    if (ctU.includes('multipart/form-data')) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const publicDirU = path.join(process.cwd(), '..', 'public', relDirU);
