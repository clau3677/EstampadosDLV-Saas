...
export default async function handleMarketing(ctx) {
  ...
  if (route === '/marketing/contest' && method === 'GET') {
    ...
    return json({
      contest: {
        ...
        prize: contest.prize, rules: contest.rules, status: contest.status,
  ...
  if (route === '/marketing/contest/upload-proof' && method === 'POST') {
    ...
    // En producción (output: standalone) process.cwd() es .next/standalone y su
    // carpeta public NO es servida por nginx. Se guarda además en el public raíz
    // del proyecto (servido por el alias /uploads/ de nginx) para que siempre se vea.
    ...
    // 1) Public raíz (nginx /uploads/ alias) — principal para visibilidad
    const publicDir = path.join(process.cwd(), '..', 'public', rel);
    await fs.mkdir(publicDir, { recursive: true });
    await fs.writeFile(path.join(publicDir, fileName), fileBuf);
    // 2) También dentro del output de Next (por si acaso)
    try {
      const standaloneDir = path.join(process.cwd(), 'public', rel);
      await fs.mkdir(standaloneDir, { recursive: true });
      await fs.writeFile(path.join(standaloneDir, fileName), fileBuf);
    } catch { /* noop: standalone dir no existe en desarrollo */ }
    await fs.mkdir(path.join(process.cwd(), 'public', rel), { recursive: true }).catch(() => {});
    await fs.writeFile(path.join(process.cwd(), 'public', rel, fileName), fileBuf).catch(() => {});
        ...
        if (targetCL >= CONTEST_START_MS && targetCL <= contestEnd) {
          ...
          if (dupCount === 0) {
            ...
            const contestImages = [
              ...
              '/uploads/contest/premio-poleron.png',
              '/uploads/contest/premio-polera.png',
              '/uploads/contest/premio-gorra.png',
            ];
            ...
            const contestCaption =
              ...
              `🥇 1er lugar: Polerón personalizado\n` +
              `🥈 2do lugar: Polera personalizada\n` +
              `🥉 3er lugar: Gorra personalizada\n\n` +
              ...
              `⏰ El sorteo termina el 12 de noviembre de 2026\n\n` +
            ...
            const contestPost = {
              ...
              altText: 'Concurso Estampados DLV: gana un polerón, polera o gorra personalizada',
              tone: 'promocional',
  ...
  if (route === '/marketing/contest/pick-winners-auto' && method === 'POST') {
    ...
    const prizeByRank = { first: { rank: '1er lugar', prize: 'Polerón personalizado' }, second: { rank: '2do lugar', prize: 'Polera personalizada' }, third: { rank: '3er lugar', prize: 'Gorra personalizada' } };
  ...
  // --- GET /marketing/contest/drawnames → nombres para el sorteo grabable (público, solo identificador seguro) ---
  if (route === '/marketing/contest/drawnames' && method === 'GET') {
    ...
    const participants = await db.collection(COLLECTIONS.CONTEST_PARTICIPANTS)
      .find({ contestId: contest.id, name: { $ne: '' }, name: { $exists: true } })
      .project({ name: 1, city: 1 })
      .toArray();
    const drawnames = participants.map((p) => {
      const first = (p.name || '').trim().split(' ')[0] || 'Participante';
      return { label: `${first} (${(p.city || 'Chile').trim().slice(0, 30)})` };
    });
    return json({ drawnames, contestId: contest.id, contestTitle: contest.title });
  }
