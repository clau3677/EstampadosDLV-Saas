...
export default async function handleMarketing(ctx) {
  ...
  // ==========================================================================
  // CONCURSOS / GIVEAWAYS (sistema automático de sorteos)
  // ==========================================================================

  // --- GET /marketing/contest → datos del concurso activo (público) ---
  if (route === '/marketing/contest' && method === 'GET') {
    const contest = await db.collection(COLLECTIONS.CONTESTS)
      .findOne({ status: 'active' });
    if (!contest) {
      return json({ contest: null, participantCount: 0 });
    }
  ...
  // ==========================================================================
  // CONCURSO: SUBIDA DE COMPROBANTES (capturas virales)
  // Flujo: compartir #1 → captura → compartir #2 → captura → seguir redes →
  //        captura → formulario final
  // ==========================================================================

  // --- POST /marketing/contest/upload-proof → subir captura de requisito viral ---
  // FormData: { email (requerido, identifica al participante), proofType: share1|share2|follow, file (imagen), sharedNetworks: JSON ['facebook','whatsapp'] }
  if (route === '/marketing/contest/upload-proof' && method === 'POST') {
    ...
    if (!['share1', 'share2', 'follow'].includes(proofType)) {
      return err('proofType debe ser share1, share2 o follow', 400);
    }
    ...
    // Buscar concurso activo y participante (ya existente por email o temporal creado aquí)
    ...
    // Guardar en disco: /public/uploads/contest-proofs/{email-safe}/{proofType}/{uuid}.ext
    const emailSafe = email.replace(/[^a-z0-9@._-]/g, '_').slice(0, 80);
    const proofDir = path.join(process.cwd(), 'public', 'uploads', 'contest-proofs', emailSafe, proofType);
    ...
    await fs.writeFile(path.join(proofDir, fileName), Buffer.from(await file.arrayBuffer()));
    const proofUrl = `/uploads/contest-proofs/${emailSafe}/${proofType}/${fileName}`;

    // Actualizar (o crear) el participante con la prueba viral
    ...
    if (participant) {
      ...
      const updates = {};
      if (proofType === 'share1') {
        updates.proofShare1Url = proofUrl;
        updates.proofShare1At = new Date();
        updates.sharedNetworks = sharedNetworks.length ? sharedNetworks : (participant.sharedNetworks || []);
      } else if (proofType === 'share2') {
        updates.proofShare2Url = proofUrl;
        updates.proofShare2At = new Date();
      } else if (proofType === 'follow') {
        updates.proofFollowUrl = proofUrl;
        updates.proofFollowAt = new Date();
        updates.followingNetworks = true;
      }
      await participantCol.updateOne({ id: participant.id }, { $set: updates });
    } else {
      // Crear participante temporal con la primera prueba
      const doc = {
        id: uuidv4(),
        ...
        city: '',
        designIdea: '',
        proofShare1Url: proofType === 'share1' ? proofUrl : null,
        proofShare1At: proofType === 'share1' ? new Date() : null,
        proofShare2Url: proofType === 'share2' ? proofUrl : null,
        proofShare2At: proofType === 'share2' ? new Date() : null,
        proofFollowUrl: proofType === 'follow' ? proofUrl : null,
        proofFollowAt: proofType === 'follow' ? new Date() : null,
        sharedNetworks: proofType === 'share1' && sharedNetworks.length ? sharedNetworks : [],
        followingNetworks: proofType === 'follow',
        viralComplete: false,
        createdAt: new Date(),
      };
    ...
    // Releer participante para calcular si completó los requisitos virales
    ...
    return json({
      ...
      proofUrl,
      proofType,
      viralComplete: viralComplete || !!updated.viralComplete,
      hasShare1: !!updated.proofShare1Url,
      hasShare2: !!updated.proofShare2Url,
      hasFollow: !!updated.proofFollowUrl,
    });
  }

  // --- GET /marketing/contest/progress → progreso viral de un participante (público, por email) ---
  if (route === '/marketing/contest/progress' && method === 'GET') {
    const url = new URL(request.url);
    const email = (url.searchParams.get('email') || '').trim().toLowerCase();
    if (!email || !email.includes('@')) return err('email requerido', 400);

    const contest = await db.collection(COLLECTIONS.CONTESTS).findOne({ status: 'active' });
    if (!contest) return json({ viralComplete: false, hasShare1: false, hasShare2: false, hasFollow: false });

    const participant = await db.collection(COLLECTIONS.CONTEST_PARTICIPANTS)
      .findOne({ contestId: contest.id, email });
    if (!participant) {
      return json({ viralComplete: false, hasShare1: false, hasShare2: false, hasFollow: false });
    }
    return json({
      viralComplete: !!participant.viralComplete,
      hasShare1: !!participant.proofShare1Url,
      hasShare2: !!participant.proofShare2Url,
      hasFollow: !!participant.proofFollowUrl,
      sharedNetworks: participant.sharedNetworks || [],
      registered: !!(participant.name && participant.name.trim()),
    });
  }

  // --- POST /marketing/contest/participate → registrar participante (público) ---
  if (route === '/marketing/contest/participate' && method === 'POST') {
    const body = await request.json();
    const { name, email, phone, city, designIdea } = body;
    if (!name || !email || !phone) {
      return err('Nombre, email y teléfono son obligatorios');
    }
    if (email.length > 100 || name.length > 100 || phone.length > 20) {
      return err('Campos demasiado largos');
    }
    ...
    const contest = await db.collection(COLLECTIONS.CONTESTS)
      .findOne({ status: 'active' });
    if (!contest) {
      return err('No hay un concurso activo');
    }
    if (new Date() > contest.endDate) {
      return err('El concurso ya terminó');
    }

    // Evitar duplicados por email
    const existing = await db.collection(COLLECTIONS.CONTEST_PARTICIPANTS)
      .findOne({ contestId: contest.id, email: email.toLowerCase().trim() });
    ...
    // Si ya existía (temporal por capturas), actualizar; si no, crear con capturas existentes
    if (existing) {
      await db.collection(COLLECTIONS.CONTEST_PARTICIPANTS).updateOne(
        { id: existing.id },
        {
          $set: {
            ...
            designIdea: (designIdea || '').trim().slice(0, 500),
  ...
  // --- POST /marketing/contest/create → crear concurso (admin) ---
