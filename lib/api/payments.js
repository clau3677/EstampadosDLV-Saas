...
// Idempotente: si ya está pagado, no cambia nada.
// ----------------------------------------------------------------------------
async function consumeReservedStockForOrder(db, orderId, orderNumber, provider) {
  ...
  for (const it of items) {
    ...
    const newReserved = Math.max(0, (stockRow.reservedQuantity || 0) - qty);
    const newQty = Math.max(0, (stockRow.quantity || 0) - qty);
    await db.collection(COLLECTIONS.COMMERCIAL_STOCK).updateOne(
      { id: stockRow.id },
      { $set: { quantity: newQty, reservedQuantity: newReserved, updatedAt: now } },
    );
    await db.collection(COLLECTIONS.STOCK_MOVEMENTS).insertOne({
      id: uuidv4(),
      type: 'commercial_out',
      reference: 'order_paid',
      referenceId: orderId,
      ...
      balanceAfter: newQty,
      operatorId: null,
      reason: `Consumo de stock por pago confirmado (${provider || 'manual'}) - pedido ${orderNumber}`,
      createdAt: now,
    });
  }
}
...
async function markOrderPaid(db, orderId, provider, transactionRef, extra = {}) {
  ...
  try {
    ...
    if (order) {
      await consumeReservedStockForOrder(db, orderId, order.orderNumber, provider);
    }
  } catch (e) {
    console.warn(`[payments/markOrderPaid] consumeReservedStock failed:`, e?.message);
  }
}
...
export default async function handlePayments(ctx) {
  ...
  if (route === '/payments/mercadopago/webhook' && method === 'POST') {
    ...
    try {
      ...
      // Aun así, 200 para que MP no reintente indefinidamente
