...
function normalizeState(value = {}) {
  const legacyStuck = value.running === true && value.version !== 2;
  return {
    version: 2,
    running: legacyStuck ? false : Boolean(value.running),
    paused: legacyStuck ? true : Boolean(value.paused),
    completed: Boolean(value.completed),
    startedAt: value.startedAt || null,
    finishedAt: value.finishedAt || null,
    updatedAt: value.updatedAt || null,
    heartbeatAt: value.heartbeatAt || null,
    error: legacyStuck ? 'Sincronización anterior interrumpida; lista para reanudar.' : (value.error || null),
