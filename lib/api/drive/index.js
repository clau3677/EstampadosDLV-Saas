...
// Concurrencia moderada: acelera I/O de Drive sin saturar CPU, memoria ni disco.
// Se puede ajustar en producción con DRIVE_SYNC_CONCURRENCY (1–6).
const DRIVE_SYNC_CONCURRENCY = Math.min(6, Math.max(1, Number.parseInt(process.env.DRIVE_SYNC_CONCURRENCY || '3', 10) || 3));
...
async function syncOneFile({ drive, assets, library, f, folderId, folderName, globalTags, stats, details }) {
...
export default async function handleDrive(ctx) {
  ...
  if (route === '/drive/sync' && method === 'POST') {
    ...
    try {
      ...
      if (syncInFlight) {
        return err('La sincronización ya está en curso', 409);
      }
      syncInFlight = true;
      ...
      // Protección de recuperación: no eliminar activos fuera de las carpetas seleccionadas.
      // La ausencia en este lote no demuestra que el archivo haya sido borrado de Drive.
      // Los borrados deben hacerse únicamente mediante una acción administrativa explícita.
      const stale = [];
