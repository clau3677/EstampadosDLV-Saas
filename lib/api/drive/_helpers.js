...
export async function getOAuthClient(db, adminId) {
  ...
  const expiryMs = conn.expiryDate || 0;
  if (refreshTok?.refresh_token && (!accessTok?.access_token || expiryMs - 60000 < nowMs)) {
    try {
      const res = await oauth2Client.refreshAccessToken();
      oauth2Client.setCredentials(res.credentials);
    } catch (e) {
      console.warn("[drive] token refresh failed:", e.message);
    }
  }
