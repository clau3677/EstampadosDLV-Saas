...
import { ensureFulfillmentForOrder } from './fulfillment';
...
export default async function handleGangSheets(ctx) {
  ...
  // Capturar info del usuario autenticado si existe
  ...
  if (user) {
    ...
    await db.collection(COLLECTIONS.ORDERS).updateOne(
      { id: orderId },
      { $set: { customerId: user.id, customerSnapshot: order.customerSnapshot } }
    );
    gangSheet.userId = user.id;
    await db.collection(COLLECTIONS.GANG_SHEETS).updateOne({ id: gangSheetId }, { $set: { userId: user.id } });
  }
