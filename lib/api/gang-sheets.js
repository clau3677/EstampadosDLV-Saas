...
import { notifyNewGangSheetByEmail } from '@/lib/email/notifications';
import { notifyNewGangSheetByWhatsApp } from '@/lib/whatsapp/notifications';
...
export default async function handleGangSheets(ctx) {
  ...
  // Un Gang Sheet siempre crea un pedido y debe quedar ligado a una cuenta.
  // La validación se hace en servidor para impedir pedidos anónimos mediante
  // POST directo, aunque el cliente sea manipulado o se omita la interfaz.
  const user = getUserFromRequest(request);
  if (!user?.id) return err('Debes iniciar sesión o registrarte para usar Gang Sheet Builder', 401);
  ...
  // --- Notificaciones al equipo admin (email + WhatsApp) ---
  ...
  // Enviar notificaciones (best-effort, nunca fallan el request)
  try { await notifyNewGangSheetByEmail({ order, gangSheet: gsInfo }); } catch (e) { console.warn('[gang-sheets] email notify failed:', e.message); }
  try { await notifyNewGangSheetByWhatsApp({ order, gangSheet: gsInfo }); } catch (e) { console.warn('[gang-sheets] wa notify failed:', e.message); }

  // (M1) Exportación inmediata: renderiza el PNG y lo envía al hot folder
  // Non-blocking: si falla, se registra en pre_press_exports con status=failed
  autoExportForOrder(db, orderId)
    .then((r) => {
      if (r?.attempts) console.log(`[gang-sheets] pre-press exported ${r.attempts} gang-sheet(s) for ${orderNumber}`);
    })
    .catch((e) => console.warn('[gang-sheets] pre-press auto-export failed:', e.message));
