  ...
  if (route === '/products' && method === 'GET') {
    ...
    if (lite) {
      ...
      const items = await db.collection(COLLECTIONS.PRODUCTS).find(
        { active: true },
        { projection: { id: 1, sku: 1, name: 1, slug: 1, category: 1, subcategory: 1, images: 1, basePrice: 1, active: 1, ...(includeVariants ? { variants: 1 } : {}) } }
      ).sort({ name: 1 }).toArray();
      return json(includeVariants ? items.map(p => ({ ...strip(p), variants: (p.variants || []).map(v => ({ id: v.id, name: v.name, price: Number(v.price) || 0 })) })) : items);
    }
    ...
    if (shop) {
      const items = await db.collection(COLLECTIONS.PRODUCTS).find(
        { active: true },
        { projection: { id: 1, name: 1, slug: 1, category: 1, subcategory: 1, images: 1, basePrice: 1, description: 1, featured: 1, active: 1, variants: { $slice: 1 } } }
