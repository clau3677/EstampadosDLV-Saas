...
export default async function handleProducts(ctx) {
  ...
  const isAdmin = (() => { const u = getUserFromRequest(request); return !!u && u.role === 'admin'; })();
  ...
  if (route === '/products' && method === 'GET') {
    ...
    if (lite) {
      ...
      const items = await db.collection(COLLECTIONS.PRODUCTS).find(
        { active: true },
        { projection: { id: 1, sku: 1, name: 1, slug: 1, category: 1, subcategory: 1, images: 1, basePrice: 1, baseCompareAtPrice: 1, productType: 1, active: 1, ...(includeVariants ? { variants: 1 } : {}) } }
      ).sort({ name: 1 }).toArray();
      return json(includeVariants ? items.map(p => ({ ...strip(p), variants: (p.variants || []).map(v => ({ id: v.id, name: v.name, price: Number(v.price) || 0, compareAtPrice: Number(v.compareAtPrice) || 0 })) })) : items);
    }
    ...
    if (shop) {
      const items = await db.collection(COLLECTIONS.PRODUCTS).find(
        { active: true },
        { projection: { id: 1, name: 1, slug: 1, category: 1, subcategory: 1, images: 1, basePrice: 1, baseCompareAtPrice: 1, description: 1, featured: 1, active: 1, variants: { $slice: 1 } } }
      ).sort({ createdAt: -1 }).toArray();
      // Enriquecer con count de variantes y precio mínimo
      const shopItems = items.map(p => ({
        id: p.id,
        name: p.name,
        slug: p.slug,
        category: p.category,
        ...
        baseCompareAtPrice: p.baseCompareAtPrice,
        description: p.description,
        featured: p.featured,
        active: p.active,
        variantCount: p.variants?.length || 1,
        minPrice: p.variants?.[0]?.price || p.basePrice,
        minCompareAtPrice: p.variants?.[0]?.compareAtPrice || p.baseCompareAtPrice || 0,
      }));
      return json(shopItems);
    }
    const items = await db.collection(COLLECTIONS.PRODUCTS).find({}).sort({ createdAt: -1 }).toArray();
    if (isAdmin) return json(strip(items));
    return json(items.map(({ digitalAssets, ...product }) => strip({ ...product, productType: product.productType === 'digital' ? 'digital' : 'physical' })));
  }
  ...
  if (route === '/products' && method === 'POST') {
    ...
    const { name, sku, category, subcategory, description, basePrice, baseCompareAtPrice, cost, variants, images, productType, digitalAssets } = body;
    if (!name || !category) return err('name y category son requeridos');
    if (productType === 'digital') {
      const assets = Array.isArray(digitalAssets) ? digitalAssets : [];
      const validAsset = assets.some(a => a && ((a.kind === 'link' && a.url) || (a.kind !== 'link' && a.storageKey)));
      if (!validAsset) return err('Un producto digital debe tener al menos un archivo cargado o un enlace de descarga válido');
    }
    ...
    // Los nombres pueden repetirse legítimamente (por ejemplo, distintas
    // colecciones o colores). El índice único de Mongo exige que el slug no
    // se repita, por lo que generamos un sufijo estable antes de insertar.
    const baseSlug = slugify(name) || `producto-${Date.now()}`;
    let slug = baseSlug;
    let slugSuffix = 2;
    while (await db.collection(COLLECTIONS.PRODUCTS).findOne({ slug })) {
    ...
    const processedVariants = (Array.isArray(variants) && variants.length > 0 ? variants : [
      ...
      compareAtPrice: Number(v.compareAtPrice) > 0 ? Number(v.compareAtPrice) : (Number(baseCompareAtPrice) > 0 ? Number(baseCompareAtPrice) : 0),
    ...
    const productDoc = {
      ...
      baseCompareAtPrice: Number(baseCompareAtPrice) > 0 ? Number(baseCompareAtPrice) : 0,
      cost: Number(cost) || 0,
      ...
      digitalAssets: productType === 'digital' && Array.isArray(digitalAssets) ? digitalAssets.map(a => ({ ...a, id: a.id || uuidv4(), kind: a.kind === 'link' ? 'link' : 'file' })) : [],
  ...
  if (route === '/products' && method === 'PATCH') {
    if (!isAdmin) return err('No autorizado', 403);
    const body = await request.json();
    const { id, variants, productType, digitalAssets, baseCompareAtPrice, ...rest } = body;
    if (!id) return err('id requerido');
    if (productType === 'digital' && digitalAssets !== undefined) {
      const assets = Array.isArray(digitalAssets) ? digitalAssets : [];
      const validAsset = assets.some(a => a && ((a.kind === 'link' && a.url) || (a.kind !== 'link' && a.storageKey)));
      if (!validAsset) return err('Un producto digital debe tener al menos un archivo cargado o un enlace de descarga válido');
    }
    ...
    const updates = { ...rest, baseCompareAtPrice: baseCompareAtPrice === undefined ? (Number(existing.baseCompareAtPrice) || 0) : (Number(baseCompareAtPrice) > 0 ? Number(baseCompareAtPrice) : 0), updatedAt: new Date() };
    if (productType !== undefined) updates.productType = productType === 'digital' ? 'digital' : 'physical';
    if (digitalAssets !== undefined) updates.digitalAssets = Array.isArray(digitalAssets) ? digitalAssets.map(a => ({ ...a, id: a.id || uuidv4(), kind: a.kind === 'link' ? 'link' : 'file' })) : [];
      ...
      const newVariants = variants.map((v, i) => {
        ...
        return {
          ...
          compareAtPrice: v.compareAtPrice === undefined
            ? (Number(old?.compareAtPrice) || 0)
            : (Number(v.compareAtPrice) > 0 ? Number(v.compareAtPrice) : (Number(baseCompareAtPrice) > 0 ? Number(baseCompareAtPrice) : 0)),
          attributes: v.attributes || {},
        };
      });
    ...
    for (const item of items) {
      try {
        ...
        const processedVariants = rawVariants.map((v, i) => ({
          ...
          compareAtPrice: Number(v.compareAtPrice) > 0 ? Number(v.compareAtPrice) : (Number(item.baseCompareAtPrice) > 0 ? Number(item.baseCompareAtPrice) : 0),
          attributes: v.attributes || {},
          _initialStock: Number(v.initialStock) || 0,
        }));
        ...
        await db.collection(COLLECTIONS.PRODUCTS).insertOne({
          ...
          basePrice: Number(item.basePrice) || 0, baseCompareAtPrice: Number(item.baseCompareAtPrice) > 0 ? Number(item.baseCompareAtPrice) : 0, cost: Number(item.cost) || 0,
          variants: processedVariants.map(({ _initialStock, ...v }) => v),
          active: true,
