...
export default async function handleOrders(ctx) {
  ...
  if (route === '/orders/confirm-payment' && method === 'POST') {
    ...
    // El error se informa al operador: el helper revierte reservas parciales
    // y deja el pedido reintentable en vez de ocultar la inconsistencia.
    try {
      await consumeReservedStockForOrder(db, id, order.orderNumber);
    } catch (e) {
      console.error('[orders/confirm-payment] consumeReservedStock failed:', e?.message);
      return err(`Pago confirmado, pero el consumo de stock requiere revisión: ${e.message}`, 409);
    }
