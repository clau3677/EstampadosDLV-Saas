...
// Consume el stock reservado por un pedido pagado: libera la reserva Y descuenta el stock físico.
// Llamada cuando el pedido pasa a PAID/IN_PRODUCTION (pago confirmado).
async function consumeReservedStockForOrder(db, orderId, orderNumber) {
  ...
  for (const it of items) {
    ...
    await db.collection(COLLECTIONS.STOCK_MOVEMENTS).insertOne({
      ...
      balanceAfter: newQty,
      operatorId: null,
      reason: `Consumo de stock por pedido pagado ${orderNumber}`,
      createdAt: now,
    });
  }
}
