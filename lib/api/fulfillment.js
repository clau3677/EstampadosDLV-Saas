...
async function scheduleReviewForCompletedOrder(db, orderId) {
  if (!db || !orderId) return null;
  const order = await db.collection(COLLECTIONS.ORDERS).findOne({ id: orderId });
  if (!order || order.status === 'cancelled') return null;
  return scheduleReviewRequest(db, order);
}

function publicReviewRequest(row) {
  if (!row) return null;
  return strip({
    id: row.id,
    orderId: row.orderId,
...
export default async function handleFulfillment(ctx) {
  ...
  if (route === '/fulfillment/customer' && method === 'GET') {
    ...
    if ([FULFILLMENT_STATUS.DELIVERED, FULFILLMENT_STATUS.PICKED_UP].includes(fulfillment.status)) {
      await scheduleReviewForCompletedOrder(db, order.id).catch(e => console.warn('[fulfillment] review schedule:', e.message));
    }
    const reviewRequest = await db.collection(COLLECTIONS.REVIEW_REQUESTS).findOne({ orderId: order.id });
    ...
    return json({
      ...
      reviewRequest: publicReviewRequest(reviewRequest),
  ...
  if (route === '/fulfillment/pickup' && method === 'POST') {
    ...
    scheduleReviewForCompletedOrder(db, fulfillment.orderId)
      .catch(e => console.warn('[fulfillment] review schedule pickup:', e.message));
    const updated = await db.collection(COLLECTIONS.FULFILLMENTS).findOne({ id });
    return json({ ok: true, fulfillment: strip(updated) });
  }
  ...
  if (route === '/fulfillment/transition' && method === 'POST') {
    ...
    if ([FULFILLMENT_STATUS.DELIVERED, FULFILLMENT_STATUS.PICKED_UP].includes(toStatus)) {
      scheduleReviewForCompletedOrder(db, fulfillment.orderId)
        .catch(e => console.warn('[fulfillment] review schedule transition:', e.message));
    }
