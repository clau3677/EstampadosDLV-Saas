...
function buildProductPrompt({ product, tone, extraContext }) {
  const variantSummary = (product.variants || []).slice(0, 6).map(v => {
    const specs = [];
    if (v.attributes?.size) specs.push(`talla ${v.attributes.size}`);
    if (v.attributes?.color) specs.push(`color ${v.attributes.color}`);
    return specs.join(', ');
  ...
  const priceHint = product.price
    ...
    : (product.basePrice ? `Precio referencial: $${Number(product.basePrice).toLocaleString('es-CL')} CLP.` : '');

  return `Genera una landing page SEO en JSON estricto DEDICADA al siguiente producto de "Estampados DLV". Este es un TALLER de personalización, así que la mayoría de productos que vendemos son PRENDAS BASE que el cliente puede comprar tal cual O pedir que se les agregue estampado.

═══════════════════════════════════════════════════
DATOS EXACTOS DEL PRODUCTO (usa SÓLO esta info):
═══════════════════════════════════════════════════
- Nombre: ${product.name}
- Categoría: ${product.category || 'producto'}${product.subcategory ? ' > ' + product.subcategory : ''}
- Descripción oficial: "${(product.description || '').trim() || '(sin descripción)'}"
- ${priceHint}
- Variantes: ${variantSummary}
${product.material ? `- Material: ${product.material}\n` : ''}${product.brand ? `- Marca: ${product.brand}\n` : ''}
${tone ? `Tono adicional: ${tone}.\n` : ''}${extraContext ? `Contexto extra proporcionado por admin: ${extraContext}\n` : ''}

═══════════════════════════════════════════════════
REGLAS DE HONESTIDAD (críticas — no romper):
═══════════════════════════════════════════════════
1. USA SÓLO las características que aparecen literalmente en la "Descripción oficial" de arriba.
   - Si no dice "algodón peinado" NO digas "algodón peinado".
   - Si NO dice "estampado DTF" NO digas que viene con DTF.
   - Si NO dice tallas/colores específicos NO los inventes.

2. Distingue CLARAMENTE 2 conceptos:
   • EL PRODUCTO = la prenda física que llega al cliente exactamente como se describe arriba (ej. gorra con bordado, polera lisa blanca, camisa sin diseño).
   • EL SERVICIO DE DLV = personalización adicional opcional con DTF, serigrafía, sublimación o bordado. Esto se paga aparte, es un valor añadido, NO viene incluido en el producto.

3. El H1 debe describir el PRODUCTO REAL con su característica más distintiva.
   ✅ CORRECTO: "Gorra trucker con bordado Eagle · Estampados DLV Chile"
   ✅ CORRECTO: "Polera de algodón lisa para estampar · Estampados DLV"
   ❌ INCORRECTO: "Gorra Animal Malla (Eagle) · Personalización DTF y más"
     (implica erróneamente que la gorra viene con DTF)

4. Si el producto YA TIENE un diseño (bordado, print, parche), NO digas que puede "estampar el bordado" ni cosas que contradigan el diseño existente. Sí puedes decir "puedes agregar más personalización" o "lo llevamos base y le sumamos tu diseño en otra zona".

5. Si el producto es una PRENDA LISA (sin diseño), enfoca el body en: (a) características físicas y (b) que es ideal como base para estampar.

═══════════════════════════════════════════════════
REGLAS DE FORMATO:
═══════════════════════════════════════════════════
- Devuelve SOLO un objeto JSON válido, sin markdown, sin comentarios, sin texto extra.
- Español chileno neutro. Sin emojis en H1/meta title/meta description/slug.
- Body con 3 párrafos separados por \\n\\n:
    § Párrafo 1: Descripción física del producto usando SÓLO datos reales (materiales, medidas, diseño existente).
    § Párrafo 2: Usos concretos (regalo, uniforme, colección de tienda, uso diario) — sin inventar.
    § Párrafo 3: Servicio adicional de Estampados DLV (mencionar que además ofrecemos personalizar con DTF/sublimación si el cliente lo pide, siempre como servicio EXTRA opcional).

═══════════════════════════════════════════════════
FORMATO EXACTO REQUERIDO:
═══════════════════════════════════════════════════
{
  "slug": "kebab-case-derivado-del-nombre-max-60-chars",
  "h1": "Título 45-70 chars enfocado en LA CARACTERÍSTICA REAL más distintiva del producto",
  "intro": "1-2 frases 100-160 chars describiendo QUÉ ES el producto (no lo que se puede hacer con él)",
  "body": "Párrafo 1 (qué es y de qué está hecho)\\n\\nPárrafo 2 (usos)\\n\\nPárrafo 3 (opcional: servicio extra de personalización DLV)",
  "ctaText": "Texto del botón CTA 2-4 palabras (ej: 'Comprar ahora', 'Pedir el mío')",
  "metaTitle": "SEO title máx 60 chars, honesto sobre lo que es",
  "metaDescription": "SEO description 130-155 chars, describe el producto real",
  "keywords": ["palabras del nombre real", "categoria", "material", "chile", "estampados dlv", "kw6"]
}`;
}
...
export default async function handleLandings(ctx) {
  ...
  // Body: { productId, tone?, extraContext?, overrides?: { slug, h1, ... }, regenerate?: true }
  if (route === '/landings/from-product' && method === 'POST') {
    if (!llmIsConfigured()) {
      return err('IA no configurada. Falta MINIMAX_API_KEY en .env', 503);
    }
    const body = await request.json().catch(() => ({}));
    const { productId, tone, extraContext, overrides, regenerate } = body || {};
    if (!productId) return err('productId requerido');

    const product = await db.collection(COLLECTIONS.PRODUCTS).findOne({ id: productId });
    if (!product) return err('Producto no encontrado', 404);

    // Prevenir landings duplicadas — salvo que se pida regenerar explícitamente
    ...
    // Si es regeneración → actualizamos el doc existente en vez de crear uno nuevo
    if (existing && regenerate) {
      // Slug: si el nuevo choca con OTRO (que no sea el nuestro), añadir sufijo
      let finalSlug = merged.slug;
      const collision = await db.collection(COLLECTIONS.LANDING_PAGES).findOne({
        slug: finalSlug,
        id: { $ne: existing.id },
      });
      if (collision) finalSlug = `${merged.slug}-${product.id.slice(0, 6)}`;
      ...
      const updateFields = {
        ...
        intro: merged.intro || '',
        body: merged.body || '',
        ctaText: merged.ctaText || existing.ctaText || 'Comprar ahora',
        metaTitle: merged.metaTitle || '',
        metaDescription: merged.metaDescription || '',
        ogImage: merged.ogImage || existing.ogImage,
        heroImage: merged.heroImage || existing.heroImage,
        keywords: Array.isArray(merged.keywords) ? merged.keywords : existing.keywords || [],
        generatedByAI: true,
        generationMetadata: {
          ...
          regenerated: true,
          previousGeneratedAt: existing.generationMetadata?.generatedAt || null,
        },
        updatedAt: new Date(),
      };
      await db.collection(COLLECTIONS.LANDING_PAGES).updateOne(
        { id: existing.id },
        { $set: updateFields },
      );
      const updated = await db.collection(COLLECTIONS.LANDING_PAGES).findOne({ id: existing.id });
      return json({ ok: true, regenerated: true, landing: strip(updated), tookMs: generated.tookMs });
    }
