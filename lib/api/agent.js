...
import { deliverQuote, listQuoteDeliveries, confirmationFor } from '@/lib/agent/quote-delivery';
...
export default async function handleAgent(ctx) {
  ...
  // ── ENTREGA CONTROLADA DE COTIZACIONES ────────────────────────────────
  // Solo administración; el LLM nunca tiene acceso a esta ruta.
  if (route === '/agent/quote-delivery' && method === 'POST') {
    const user = getUserFromRequest(request);
    if (!user || user.role !== 'admin') return err('No autorizado', 401);
    const body = await request.json().catch(() => ({}));
    if (!body.quoteId || !body.channel || !body.confirmation) return err('quoteId, channel y confirmation requeridos', 400);
    const result = await deliverQuote({ db, quoteId: body.quoteId, channel: body.channel, confirmation: body.confirmation, actor: user.email || user.id || 'admin' });
    if (result.ok) return json(result);
    const response = { error: result.error };
    if (result.requiredConfirmation) response.requiredConfirmation = result.requiredConfirmation;
    if (result.sentAt) response.sentAt = result.sentAt;
    return json(response, { status: result.requiredConfirmation ? 409 : 400 });
  }
  if (route.startsWith('/agent/quote-delivery/') && method === 'GET') {
    const user = getUserFromRequest(request);
    if (!user || user.role !== 'admin') return err('No autorizado', 401);
    const quoteId = route.split('/')[3];
    const rows = await listQuoteDeliveries(db, quoteId);
    const quote = await db.collection('quotes').findOne({ id: quoteId }, { projection: { id: 1, code: 1, clientEmail: 1, clientPhone: 1, delivery: 1 } });
    return json({ quote, confirmation: quote ? confirmationFor(quote.code) : null, deliveries: strip(rows) });
  }
  ...
  // ── FOLLOW-UPS Y MÉTRICAS COMERCIALES ─────────────────────────────────
  if (route === '/agent/followups' && method === 'GET') {
    const url = new URL(request.url);
    const status = url.searchParams.get('status');
    const limit = Math.min(200, Math.max(1, parseInt(url.searchParams.get('limit') || '100', 10)));
    const filter = status ? { status } : {};
    const rows = await db.collection('agent_followup_tasks')
      .find(filter).sort({ dueAt: 1 }).limit(limit).toArray();
    return json(strip(rows));
  }
  if (route.startsWith('/agent/followups/') && method === 'PATCH') {
    const id = route.split('/')[3];
    const body = await request.json().catch(() => ({}));
    const allowed = ['status', 'priority', 'dueAt', 'reason'];
    const $set = { updatedAt: new Date() };
    for (const key of allowed) {
      if (body[key] === undefined) continue;
      if (key === 'dueAt') {
        const parsed = new Date(body[key]);
        if (Number.isNaN(parsed.getTime())) return err('dueAt inválida');
        $set[key] = parsed;
      } else if (key === 'reason') {
        $set[key] = String(body[key]).trim().slice(0, 500);
      } else {
        $set[key] = body[key];
      }
    }
    await db.collection('agent_followup_tasks').updateOne({ id }, { $set });
    const task = await db.collection('agent_followup_tasks').findOne({ id });
    return task ? json({ ok: true, task: strip(task) }) : err('seguimiento no encontrado', 404);
  }
  if (route === '/agent/metrics' && method === 'GET') {
    const [total, aiEnabled, escalated, drafts, followups, stages, sources, toolUsage] = await Promise.all([
      db.collection('agent_conversations').countDocuments({}),
      db.collection('agent_conversations').countDocuments({ aiEnabled: true }),
      db.collection('agent_conversations').countDocuments({ stage: 'human_takeover' }),
      db.collection('agent_order_drafts').countDocuments({}),
      db.collection('agent_followup_tasks').countDocuments({ status: { $in: ['pending', 'open'] } }),
      db.collection('agent_conversations').aggregate([{ $group: { _id: '$stage', count: { $sum: 1 } } }, { $sort: { count: -1 } }]).toArray(),
      db.collection('agent_conversations').aggregate([{ $group: { _id: '$source', count: { $sum: 1 } } }, { $sort: { count: -1 } }]).toArray(),
      db.collection('agent_conversations').aggregate([{ $project: { items: { $objectToArray: { $ifNull: ['$toolUsage', {}] } } } }, { $unwind: '$items' }, { $group: { _id: '$items.k', count: { $sum: '$items.v' } } }, { $sort: { count: -1 } }]).toArray(),
    ]);
    return json({
      generatedAt: new Date(),
      conversations: { total, aiEnabled, escalated },
      drafts,
      pendingFollowups: followups,
      byStage: stages.map(x => ({ stage: x._id || 'sin_etapa', count: x.count })),
      bySource: sources.map(x => ({ source: x._id || 'desconocido', count: x.count })),
      toolUsage: toolUsage.map(x => ({ tool: x._id, count: x.count })),
    });
  }
  ...
  if (route === '/agent/meta/threads' && method === 'GET') {
    try {
      ...
      const { syncThreads } = await import('../meta-inbox');
      // La sincronización ya valida el canal con Meta. Evitar una segunda llamada
      // idéntica, porque duplica la latencia y puede producir falsos timeouts.
      const conversations = await syncThreads(channel, { limit: 10 });
  ...
  if (route === '/agent/meta/send' && method === 'POST') {
    try {
      ...
      if (e?.code === 'META_WINDOW_24H') {
        return json({
          ok: false,
          window24h: true,
          error: 'Meta no permite responder: la última actividad del cliente fue hace más de 24 horas. Espera a que el cliente escriba de nuevo (se reabre la ventana) o responde por WhatsApp.',
        });
      }
