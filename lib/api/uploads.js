...
export default async function handleUploads(ctx) {
  ...
  if (route === '/uploads/image' && method === 'POST') {
    ...
    // Absoluto (mismo motivo que UPLOAD_DIR: standalone + nginx sirven desde /var/www/estampadosdlv/public)
    const IMAGES_DIR = path.resolve(process.env.PUBLIC_ROOT || '/var/www/estampadosdlv/public', 'uploads', folder);
