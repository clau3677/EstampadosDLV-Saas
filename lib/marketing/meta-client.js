...
// Modo directo con Page Token — no requiere OAuth ni App Review
// Variables: META_PAGE_ID y META_PAGE_ACCESS_TOKEN
export function isMetaConfigured() {
  // Funciona con OAuth (App ID + Secret) o directamente con Page Token
  const hasOAuth = !!(process.env.META_APP_ID && process.env.META_APP_SECRET);
  const hasDirect = !!(process.env.META_PAGE_ID && process.env.META_PAGE_ACCESS_TOKEN);
  return hasOAuth || hasDirect;
}

/** Publica foto usando Page Token directo (sin necesidad de OAuth). */
export async function publishFacebookPhotoDirect({ imageUrl, caption }) {
  const pageId = process.env.META_PAGE_ID;
  const pageToken = process.env.META_PAGE_ACCESS_TOKEN;
  if (!pageId || !pageToken) throw new Error('META_PAGE_ID y META_PAGE_ACCESS_TOKEN no configurados');
...
/** Publica post de texto/feed usando Page Token directo. */
export async function publishFacebookFeedDirect({ message, link }) {
  const pageId = process.env.META_PAGE_ID;
  const pageToken = process.env.META_PAGE_ACCESS_TOKEN;
  if (!pageId || !pageToken) throw new Error('META_PAGE_ID y META_PAGE_ACCESS_TOKEN no configurados');
...
/** Obtiene cuenta IG vinculada usando Page Token directo. */
export async function getInstagramAccountDirect(pageToken) {
  const pageId = process.env.META_PAGE_ID;
...
export async function graphFetch(path, { method = 'GET', token, params = {}, body = null } = {}) {
...
/** Publica una foto con caption en la página. imageUrl debe ser público.
 *  Si hay META_PAGE_ACCESS_TOKEN configurado, lo usa directamente sin OAuth.
 */
export async function publishFacebookPhoto({ pageId, pageToken, imageUrl, caption }) {
  // Si hay Page Token directo configurado, úsalo (prioridad)
  ...
  if (directToken && directPageId) {
    return publishFacebookPhotoDirect({ imageUrl, caption });
  }
  const data = await graphFetch(`/${pageId}/photos`, {
    method: 'POST',
    token: pageToken,
...
/** Publica un video en la página de Facebook usando URL pública. */
export async function publishFacebookVideoDirect({ videoUrl, caption }) {
  const pageId = process.env.META_PAGE_ID;
  const pageToken = process.env.META_PAGE_ACCESS_TOKEN;
  if (!pageId || !pageToken) throw new Error('META_PAGE_ID y META_PAGE_ACCESS_TOKEN no configurados');
  ...
  return { videoId: data.id, postId: data.post_id || data.id };
}

/** Publica un video (Reel) en Instagram Business usando URL pública. Espera a que el contenedor esté listo. */
async function waitForIGContainer(igUserId, containerId, pageToken, maxWaitMs = 30000) {
  ...
  while (Date.now() - start < maxWaitMs) {
    try {
      ...
      if (status.status_code === 'FINISHED' || status.status_code === 'EXPIRED') {
  ...
  // Si no pudimos verificar, intentar publicar de todas formas
  return true;
}

export async function publishInstagramVideoDirect({ videoUrl, caption }) {
  const pageId = process.env.META_PAGE_ID;
  const pageToken = process.env.META_PAGE_ACCESS_TOKEN;
  if (!pageId || !pageToken) throw new Error('META_PAGE_ID y META_PAGE_ACCESS_TOKEN no configurados');
  // Obtener cuenta IG
  const igData = await graphFetch(`/${pageId}`, {
    token: pageToken,
    params: { fields: 'instagram_business_account{id,username}' },
  });
  const igUserId = igData.instagram_business_account?.id;
  if (!igUserId) throw new Error('Cuenta Instagram no vinculada');
  ...
  // Esperar a que el contenedor esté listo (la API de IG necesita tiempo para procesar el video)
  await waitForIGContainer(igUserId, container.id, pageToken);
  // Publicar
