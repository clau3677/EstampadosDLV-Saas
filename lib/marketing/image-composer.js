...
// En Next.js standalone, process.cwd() puede ser .next/standalone.
// Nginx sirve /uploads desde PUBLIC_ROOT, por lo que el compositor debe
// escribir directamente en esa ubicación persistente y pública.
const PUBLIC_ROOT = process.env.PUBLIC_ROOT || '/var/www/estampadosdlv/public';
const MARKETING_DIR = process.env.MARKETING_MEDIA_DIR || path.join(PUBLIC_ROOT, 'uploads', 'marketing');
...
function buildTemplateSvg({ productImageDataUri, productName, category, benefit, campaign, badge, color, features, bottomBenefits, titleLines: suppliedTitleLines }) {
  ...
  return `<svg width="${SIZE}" height="${SIZE}" viewBox="0 0 ${SIZE} ${SIZE}" xmlns="http://www.w3.org/2000/svg">
    ...
    <rect x="610" y="132" width="560" height="620" rx="38" fill="#0A1429" opacity="0.78" stroke="${COLORS.orange}" stroke-opacity="0.42" stroke-width="2"/>
    <g filter="url(#shadow)">
    ...
    <text x="885" y="800" text-anchor="middle" font-family="${FONT}" font-size="17" font-weight="900" letter-spacing="1.2" fill="${COLORS.white}">${escapeXml(categoryText)}</text>
    ${campaignText ? `<text x="885" y="823" text-anchor="middle" font-family="${FONT}" font-size="13" font-weight="700" fill="${COLORS.orange}">${escapeXml(campaignText)}</text>` : ''}
...
export async function composePostImage({
  ...
  const written = await sharp(filePath).metadata();
  if (written.width !== SIZE || written.height !== SIZE || written.format !== 'jpeg') {
    throw new Error(`La imagen generada no cumple el formato publicable: ${fileName}`);
  }
