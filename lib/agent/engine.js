...
function buildSystemPrompt(cfg, contact) {
  ...
  const rules = cfg?.rules || [
    ...
    'Cotiza SOLO usando quote_gang_sheet o quote_solution. Nunca digas un precio que no salga de esas tools.',
    ...
    'Sigue estas etapas según corresponda: nuevo → calificacion → necesidad → propuesta → objecion → cierre → postventa. No saltes a vender sin entender la necesidad.',
    'En cada conversación identifica y guarda solo datos confirmados: rubro, necesidad, producto, técnica, cantidad, fecha, comuna y presupuesto. Usa update_sales_profile.',
    'Antes de repetir preguntas o continuar una conversación, usa get_conversation_context para revisar el perfil y los borradores existentes.',
    'Si el cliente confirma los datos de una cotización, usa prepare_quote para guardar el borrador en Ventas. Esa tool no envía mensajes, no cobra y no reserva stock. Para enviar, exige confirmación explícita del operador.',
    'Si queda una cotización pendiente o el cliente solicita que lo contacten, crea una tarea interna con create_followup_task. Esa tool no envía mensajes.',
    'Para restaurantes, colegios, empresas y emprendimientos recomienda primero ropa personalizada, ropa laboral o merchandising pertinente; no ofrezcas DTF UV o planchas textiles sin relación con la necesidad.',
  ];
...
- quote_gang_sheet / quote_solution → cotización oficial de DTF y soluciones combinadas
- prepare_quote → guarda el borrador confirmado en Ventas; no envía mensajes
...
- update_sales_profile / get_conversation_context / create_followup_task → memoria comercial y seguimiento interno
...
const STAGE_ORDER = ['nuevo', 'calificacion', 'necesidad', 'propuesta', 'objecion', 'cierre', 'postventa'];

function stageFromTool(name) {
  if (name === 'escalate_to_human') return 'human_takeover';
  if (name === 'create_order_draft') return 'cierre';
  if (['quote_gang_sheet', 'quote_solution', 'prepare_quote'].includes(name)) return 'propuesta';
  if (name === 'update_sales_profile') return 'necesidad';
  if (['search_products', 'get_product_details', 'check_stock'].includes(name)) return 'calificacion';
  return null;
}

async function recordToolOutcome(db, conversationId, name) {
  const nextStage = stageFromTool(name);
  const conversation = await db.collection('agent_conversations').findOne(
    { id: conversationId },
    { projection: { stage: 1, aiEnabled: 1 } }
  );
  if (!conversation || conversation.aiEnabled === false) return;
  const current = conversation.stage || 'nuevo';
  const shouldAdvance = nextStage && (
    nextStage === 'human_takeover' ||
    !STAGE_ORDER.includes(current) ||
    STAGE_ORDER.indexOf(nextStage) >= STAGE_ORDER.indexOf(current)
  );
  const set = { updatedAt: new Date(), lastToolAt: new Date(), lastToolName: name };
  if (shouldAdvance) set.stage = nextStage;
  await db.collection('agent_conversations').updateOne(
    { id: conversationId },
    { $inc: { toolCallCount: 1, [`toolUsage.${name}`]: 1 }, $set: set }
  );
}
  ...
  while (iter < MAX_TOOL_ITERS) {
    ...
    if (assistantMsg.tool_calls?.length) {
      ...
      for (const tc of assistantMsg.tool_calls) {
        ...
        await recordToolOutcome(db, conv.id, name);
        messages.push({
          role: 'tool',
          tool_call_id: tc.id,
          name,
