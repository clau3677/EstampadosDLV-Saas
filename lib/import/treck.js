 ...
 *
 * También normalizamos acentos (NFD + strip diacritics) para que "Ignífuga" matchee /ignif/.
 */
export function detectWorkwearType(categories = []) {
  const joined = categories.join(' ').toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  if (/ropa\s*de\s*trabajo|\/trabajo\b/.test(joined)) return 'trabajo';
  if (/ignif/.test(joined))                            return 'ignifuga';
  if (/ropa\s*tecnica|\btecnica\b/.test(joined))      return 'tecnica';
  if (/outdoor/.test(joined))                          return 'outdoor';
  return 'otros';
}
...
function extractVariantsFromItems(items = []) {
  ...
  const colorToImageMap = new Map();  // color → primera URL de imagen
  ...
  for (const it of items) {
    ...
    // Imagen específica de este SKU (VTEX: cada item tiene su foto por color)
    const skuImage = it.images?.[0]?.imageUrl || null;

    // Registrar la primera imagen para este color (para variantes que no tienen fotos propias)
    if (skuImage && color && !colorToImageMap.has(color)) {
      colorToImageMap.set(color, skuImage);
    }
    ...
    variants.push({
      ...
      image: skuImage,   // se completa después con la del color si vino null
    });

    colorsSet.add(color);
    sizesSet.add(talla);

    // Imágenes globales (dedup por URL)
  ...
  // Rellenar variant.image con la primera imagen del color si estaba vacía
  for (const v of variants) {
    if (!v.image && colorToImageMap.has(v.color)) {
      v.image = colorToImageMap.get(v.color);
    }
  }
