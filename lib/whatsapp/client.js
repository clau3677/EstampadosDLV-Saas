...
/**
 * Envía un documento desde un Buffer sin publicarlo en una URL pública.
 * El límite evita cargar archivos accidentalmente gigantes en la sesión.
 */
export async function sendDocument(rawPhone, buffer, { fileName, mimetype = 'application/pdf', caption = '' } = {}) {
  const s = getStore();
  if (s.state !== 'connected' || !s.sock) {
    throw new Error(`WhatsApp no está conectado (estado: ${s.state}). Vincula la sesión primero.`);
  }
  const jid = toWhatsappJid(rawPhone);
  if (!jid) throw new Error(`Teléfono inválido: "${rawPhone}"`);
  if (!Buffer.isBuffer(buffer) || buffer.length === 0) throw new Error('Documento PDF vacío o inválido');
  if (buffer.length > 10 * 1024 * 1024) throw new Error('Documento demasiado grande para WhatsApp (máximo 10 MB)');
  const safeName = String(fileName || 'cotizacion-estampados-dlv.pdf').replace(/[^a-zA-Z0-9._-]/g, '_');
