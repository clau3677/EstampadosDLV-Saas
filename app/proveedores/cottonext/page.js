...
export default function CottonextImportPage() {
  ...
  const loadImported = async () => {
    try {
      const r = await fetch('/api/import/cottonext/imported', { credentials: 'include' });
  ...
  const loadHistory = async () => {
    try {
      const r = await fetch('/api/import/cottonext/history', { credentials: 'include' });
      const d = await r.json();
  ...
  const runScan = async () => {
    ...
    try {
      const r = await fetch('/api/import/cottonext/scan', {
        method: 'POST',
  ...
  const runImport = async () => {
    ...
    setImportResult(null);
    try {
      const r = await fetch('/api/import/cottonext/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const runRefreshPrices = async () => {
    ...
    setRefreshing(true);
    try {
      const r = await fetch('/api/import/cottonext/refresh-prices', {
        method: 'POST',
        credentials: 'include',
      });
      const d = await r.json();
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900">Importar de Cottonext</h1>
              <p className="text-sm text-slate-500 mt-0.5">
                Sincroniza el catálogo de tu proveedor con markup +{markupPercent}% aplicado automáticamente.
              </p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button asChild variant="outline" size="sm">
            <a href="https://www.cottonext.cl/" target="_blank" rel="noopener noreferrer">
      ...
      <Tabs defaultValue="import">
        ...
        <TabsContent value="imported" className="space-y-4 mt-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold text-slate-900">Productos sincronizados con Cottonext</h2>
