...
async function handle(request, { params }) {
  ...
  try {
    ...
    // ------------------------------------------------------------
    // Landings SEO — /api/landings CRUD
    // ------------------------------------------------------------
    if (route === '/landings' && method === 'GET') {
      const url = new URL(request.url);
    ...
    if (route === '/landings' && method === 'POST') {
      const body = await request.json();
      const slug = (body.slug || '').trim().toLowerCase();
      if (!slug || !body.h1) return cors(NextResponse.json({ error: 'slug y h1 son obligatorios' }, { status: 400 }));
      if (!/^[a-z0-9-]+$/.test(slug)) return cors(NextResponse.json({ error: 'slug inválido (usa a-z, 0-9, guiones)' }, { status: 400 }));
      const dup = await db.collection(COLLECTIONS.LANDING_PAGES).findOne({ slug });
      if (dup) return cors(NextResponse.json({ error: 'Ya existe una landing con ese slug' }, { status: 409 }));
      const doc = {
        id: uuidv4(),
        slug,
    ...
    if (route === '/landings' && method === 'PATCH') {
      const body = await request.json();
      ...
      if (update.slug) {
        ...
        if (!/^[a-z0-9-]+$/.test(s)) return cors(NextResponse.json({ error: 'slug inválido' }, { status: 400 }));
        update.slug = s;
        const dup = await db.collection(COLLECTIONS.LANDING_PAGES).findOne({ slug: s, id: { $ne: body.id } });
        if (dup) return cors(NextResponse.json({ error: 'slug ya usado' }, { status: 409 }));
      }
    ...
    if (route === '/landings' && method === 'DELETE') {
      const body = await request.json();
      if (!body.id) return cors(NextResponse.json({ error: 'id requerido' }, { status: 400 }));
